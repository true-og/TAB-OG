...
public class PacketEntityView implements EntityView {
    ...
    /**
     * Loads all required classes and fields and marks class as available. If
     * something goes wrong, a warning is printed and class is not marked as
     * available.
     */
    public static void tryLoad() {
        ...
        try {
            ...
            if (BukkitReflection.is1_19_4Plus()) {
                ...
                Class<?> ClientboundBundlePacket = Class
                        .forName("net.minecraft.network.protocol.game.ClientboundBundlePacket");
                newClientboundBundlePacket = ClientboundBundlePacket.getConstructor(Iterable.class);
                ClientboundBundlePacket_packets = ReflectionUtils.getOnlyField(ClientboundBundlePacket.getSuperclass(),
                        Iterable.class);
                isBundlePacket = ClientboundBundlePacket::isInstance;
                sendAsBundle = (player, packets) -> packetSender.sendPacket(player.getPlayer(),
                        newClientboundBundlePacket.newInstance(packets));
    ...
    private static void loadEntityMetadata() throws ReflectiveOperationException {
        ...
        Class<?> entityMetadataClass = BukkitReflection.getClass("network.protocol.game.ClientboundSetEntityDataPacket",
                "network.protocol.game.PacketPlayOutEntityMetadata", "PacketPlayOutEntityMetadata",
        ...
        if (BukkitReflection.is1_19_3Plus()) {
            ...
            Constructor<?> constructor = entityMetadataClass.getConstructor(int.class, DataWatcher.DataWatcher,
                    boolean.class);
    ...
    private static void loadEntityDestroy() throws ReflectiveOperationException {
        ...
        EntityDestroyClass = BukkitReflection.getClass("network.protocol.game.ClientboundRemoveEntitiesPacket",
                "network.protocol.game.PacketPlayOutEntityDestroy", "PacketPlayOutEntityDestroy",
        ...
        try {
            ...
            destroyEntities = (player, ids) -> packetSender.sendPacket(player.getPlayer(),
                    constructor.newInstance(new Object[]
                    { ids }));
            if (BukkitReflection.getMinorVersion() >= 17) {

                getDestroyedEntities = packet -> ((List<Integer>) entities.get(packet)).stream().mapToInt(i -> i)
                        .toArray();
    ...
    private static void loadEntityTeleport() throws ReflectiveOperationException {
        ...
        EntityTeleportClass = BukkitReflection.getClass("network.protocol.game.ClientboundTeleportEntityPacket",
                "network.protocol.game.PacketPlayOutEntityTeleport", "PacketPlayOutEntityTeleport",
        ...
        if (BukkitReflection.getMinorVersion() >= 17) {
            ...
            Constructor<?> newEntityArmorStand = entityArmorStand.getConstructor(world, double.class, double.class,
                    double.class);
            Method World_getHandle = BukkitReflection.getBukkitClass("CraftWorld").getMethod("getHandle");
            Object dummyEntity = newEntityArmorStand.newInstance(World_getHandle.invoke(Bukkit.getWorlds().get(0)), 0,
                    0, 0);

            Constructor<?> constructor = EntityTeleportClass
                    .getConstructor(BukkitReflection.getClass("world.entity.Entity"));
    ...
    private static void loadEntityMove() throws ReflectiveOperationException {
        ...
        PacketPlayOutEntityLook = BukkitReflection.getClass("network.protocol.game.ClientboundMoveEntityPacket$Rot",
                ...
                "PacketPlayOutEntity$PacketPlayOutEntityLook", "PacketPlayOutEntityLook", "Packet32EntityLook");
        ...
        if (BukkitReflection.getMinorVersion() >= 14) {
            ...
            Constructor<?> constructor = packetPlayOutRelEntityMove.getConstructor(int.class, short.class, short.class,
                    short.class, boolean.class);
            newMovePacket = (entityId, x, y, z) -> constructor.newInstance(entityId, x.shortValue(), y.shortValue(),
                    z.shortValue(), false);
            ...
            Constructor<?> constructor = packetPlayOutRelEntityMove.getConstructor(int.class, long.class, long.class,
            ...
            PacketPlayOutEntity_Y = fields.get(1);
            PacketPlayOutEntity_Z = fields.get(2);
            Constructor<?> constructor = packetPlayOutRelEntityMove.getConstructor(int.class, byte.class, byte.class,
                    byte.class, boolean.class);
            newMovePacket = (entityId, x, y, z) -> constructor.newInstance(entityId, x.byteValue(), y.byteValue(),
                    z.byteValue(), false);
    ...
    /**
     * Loads required NMS classes, fields and methods. If it fails, exception is
     * thrown.
     *
     * @throws ReflectiveOperationException If anything fails
     */
    private static void loadEntitySpawn() throws ReflectiveOperationException {
        ...
        if (BukkitReflection.is1_20_2Plus()) {
            ...
            PacketPlayOutNamedEntitySpawn = BukkitReflection.getClass(
                    ...
                    "network.protocol.game.PacketPlayOutNamedEntitySpawn", "PacketPlayOutNamedEntitySpawn",
        ...
        if (minorVersion >= 17) {
            ...
            if (minorVersion >= 19) {
                ...
                constructor = SpawnEntityClass.getConstructor(int.class, UUID.class, double.class, double.class,
                        double.class, float.class, float.class, EntityTypes, int.class, Vec3D, double.class);
                newSpawnEntity = (id, uuid, type, l, data) -> constructor.newInstance(id, uuid, l.getX(), l.getY(),
                        l.getZ(), 0, 0, EntityTypes_ARMOR_STAND, 0, Vec3D_Empty, 0.0d);
                ...
                constructor = SpawnEntityClass.getConstructor(int.class, UUID.class, double.class, double.class,
                        double.class, float.class, float.class, EntityTypes, int.class, Vec3D);
                newSpawnEntity = (id, uuid, type, l, data) -> constructor.newInstance(id, uuid, l.getX(), l.getY(),
                        l.getZ(), 0, 0, EntityTypes_ARMOR_STAND, 0, Vec3D_Empty);
    ...
    @SneakyThrows
    @Override
    public void spawnEntity(int entityId, @NotNull UUID id, @NotNull Object entityType, @NotNull Location l,
            @NotNull EntityData data)
    {

        if (BukkitReflection.getMinorVersion() >= SPLIT_METADATA_VERSION) {

            sendAsBundle.accept(player, Arrays.asList(newSpawnEntity.apply(entityId, id, entityType, l, null),
    ...
    public Location getMoveDiff(@NotNull Object movePacket) {
        ...
        return new Location(((Number) PacketPlayOutEntity_X.get(movePacket)).intValue(),
