...
public class ReflectionUtils {
    ...
    /**
     * Returns {@code true} if class with given full name exists, {@code false} if
     * not.
     *
     ...
     * @return {@code true} if exists, {@code false} if not
     */
    public static boolean methodExists(@NotNull Class<?> clazz, @NotNull String method,
    ...
    /**
     * Returns all methods from class which return specified class type and have
     ...
     * @return List of found methods matching requirements. If nothing is found,
     *         empty list is returned.
     */
    @NotNull
    public static List<Method> getMethods(@NotNull Class<?> clazz, @NotNull Class<?> returnType,
            @NotNull Class<?>... parameterTypes)
    {
    ...
    /**
     * Returns method with specified possible names and parameters. Throws exception
     ...
     */
    @NotNull
    public static Method getMethod(@NotNull Class<?> clazz, @NotNull String[] names,
     ...
     * @throws IllegalStateException If class has more than 1 constructor or doesn't
     *                               have any
     */
    @NotNull
     ...
     * @throws IllegalStateException If more than 1 field meets the criteria or if
     *                               none do.
     */
    @NotNull
    public static Field getOnlyField(@NotNull Class<?> clazz, @NotNull Class<?> type) {
        ...
        if (list.size() != 1) {
            ...
            throw new IllegalStateException(
                    "Class " + clazz.getName() + " is expected to have 1 field of type " + type.getName() + ", but has "
    ...
    /**
     * Returns the one and only method of class with defined return type and
     * parameters. Throws IllegalStateException if more than 1 or no methods were
     * found.
     *
     * @param clazz          class to check fields of
     * @param returnType     field type to check for
     * @param parameterTypes Parameter types
     * @return The one and only method with defined return type and parameter
     * @throws IllegalStateException If more than 1 methods meets the criteria or if
     *                               none do.
     */
    @NotNull
    public static Method getOnlyMethod(@NotNull Class<?> clazz, @NotNull Class<?> returnType,
            @NotNull Class<?>... parameterTypes)
    {
        ...
        if (list.size() != 1) {
            ...
            throw new IllegalStateException("Class " + clazz.getName()
                    + " is expected to have 1 method with return type " + returnType.getName() + " and parameters "
                    + Arrays.toString(parameterTypes) + ", but has " + list.size() + ": \n"
     ...
     * @throws IllegalStateException If class has more than 1 field or has none
     */
    @NotNull
    public static Field getOnlyField(@NotNull Class<?> clazz) {

        Field[] fields = Arrays.stream(clazz.getDeclaredFields()).filter(f -> !Modifier.isStatic(f.getModifiers()))
                .toArray(Field[]::new);
        if (fields.length != 1) {
     ...
     * @param names All possible names of the field
