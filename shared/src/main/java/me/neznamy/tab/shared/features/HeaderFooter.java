        ...
        WorldSwitchListener, ServerSwitchListener, Refreshable
{
    ...
    public HeaderFooter() {
        ...
        disableChecker = new DisableChecker(getFeatureName(), disableCondition, this::onDisableConditionChange,
                p -> p.disabledHeaderFooter);
        TAB.getInstance().getFeatureManager().registerFeature(TabConstants.Feature.HEADER_FOOTER + "-Condition",
                disableChecker);
    ...
    public void onServerChange(@NotNull TabPlayer p, @NotNull String from, @NotNull String to) {
        ...
        // Velocity clears header/footer on server switch, resend regardless of whether
    ...
    private void updateProperties(TabPlayer p) {
        ...
        boolean refresh = p.setProperty(this, TabConstants.Property.HEADER,
                getProperty(p, TabConstants.Property.HEADER));
        ...
        if (refresh) {
            ...
            sendHeaderFooter(p, p.getProperty(TabConstants.Property.HEADER).get(),
    ...
    public void refresh(@NotNull TabPlayer p, boolean force) {
        ...
        sendHeaderFooter(p, p.getProperty(TabConstants.Property.HEADER).updateAndGet(),
    ...
    private String getFromConfig(TabPlayer p, String property) {
        ...
        String[] value = TAB.getInstance().getConfiguration().getUsers().getProperty(p.getName(), property,
                p.getServer(), p.getWorld());
        if (value.length > 0) {
        ...
        value = TAB.getInstance().getConfiguration().getUsers().getProperty(p.getUniqueId().toString(), property,
                p.getServer(), p.getWorld());
        if (value.length > 0) {
        ...
        value = TAB.getInstance().getConfiguration().getGroups().getProperty(p.getGroup(), property, p.getServer(),
                p.getWorld());
        if (value.length > 0) {
    ...
    @Override
    public void setHeaderAndFooter(@NotNull me.neznamy.tab.api.TabPlayer p, @Nullable String header,
