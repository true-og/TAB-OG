...
public abstract class ScoreboardLine extends TabFeature implements Line, Refreshable {
    ...
    protected ScoreboardLine(@NonNull ScoreboardImpl parent, int lineNumber, String text) {
        ...
        TAB.getInstance().getFeatureManager()
                .registerFeature(TabConstants.Feature.scoreboardScore(parent.getName(), lineNumber), scoreRefresher);
     ...
     *
     * @param viewer Player to get forced name start for
    ...
    protected String[] split(@NonNull String string, int firstElementMaxLength) {
        ...
        if (string.charAt(splitIndex - 1) == EnumChatFormat.COLOR_CHAR)
            splitIndex--;
    ...
    protected String getPlayerName(int lineNumber) {
        ...
        return EnumChatFormat.COLOR_STRING + "0123456789abcdefklmnor".charAt(lineNumber - 1)
                + EnumChatFormat.COLOR_STRING + "r";
     ...
     * @param suffix     suffix
     */
    protected void addLine(@NonNull TabPlayer p, @NonNull String fakePlayer, @NonNull String prefix,
            @NonNull String suffix)
    {

        p.getScoreboard().setScore(ScoreboardManagerImpl.OBJECTIVE_NAME, fakePlayer, getNumber(p), null, // Makes no
                                                                                                         // sense for
                                                                                                         // TAB
                scoreRefresher.getNumberFormat(p));
        p.getScoreboard().registerTeam(teamName, prefix, suffix, Scoreboard.NameVisibility.NEVER,
                Scoreboard.CollisionRule.NEVER, Collections.singletonList(fakePlayer), 0, EnumChatFormat.RESET);
        shownPlayers.add(p);
    ...
    /**
     * Splits entered text into 3 parts - prefix, name and suffix respecting all
     * limits. Returns the values as an array of 3 elements.
     *
     * @param playerNameStart forced start of name field (used to secure unique
     *                        names and line order)
     * @param text            text to display
     * @param maxNameLength   maximum length of name field
     ...
     * and second part is number format (optional)
     *
     * @param text Inputted text to categorize
     */
    ...
    protected void updateTeam(@NotNull TabPlayer player, @NotNull String prefix, @NotNull String suffix) {
        ...
        player.getScoreboard().updateTeam(teamName, prefix, suffix, Scoreboard.NameVisibility.ALWAYS,
                Scoreboard.CollisionRule.ALWAYS, 0, EnumChatFormat.RESET);
