...
public class ParseCommand extends SubCommand {
    ...
    public void execute(@Nullable TabPlayer sender, @NotNull String[] args) {
        ...
        if (!replaced.contains("%")) {
            ...
            sendMessage(sender, "&cThe provided input (" + replaced
                    + ") does not contain any placeholders, therefore there's nothing to test.");
            return;

        }

        String message = EnumChatFormat
                .color("&6Replacing placeholder &e%placeholder% &6for player &e" + target.getName())
        ...
        TabComponent colored = TabComponent
                .optimized(EnumChatFormat.color("&3Colored output: &e\"&r" + replaced + "&e\""));
        if (sender != null) {
        ...
        sendRawMessage(sender, EnumChatFormat.color("&3Raw colors: &e\"&r") + EnumChatFormat.decolor(replaced)
