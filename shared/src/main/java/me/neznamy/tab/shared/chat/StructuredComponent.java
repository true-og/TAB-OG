...
public class StructuredComponent extends TabComponent {
    ...
    public StructuredComponent(@NotNull StructuredComponent component) {
        ...
        extra = component.extra == null ? null
     ...
     * @param components Extra components
     */
    public StructuredComponent(@NotNull String text, @NotNull List<StructuredComponent> components) {

        this.text = text;
        if (components.isEmpty())
            throw new IllegalArgumentException("Unexpected empty array of components"); // exception taken from
                                                                                        // minecraft
        extra = components;
    ...
    /**
     * Returns list of extra components. If no extra components are defined, returns
     * empty list.
     *
    ...
    /**
     * Appends text to string builder, might also add color and magic codes if they
     * are different from previous component in chain.
     *
    ...
    private @NotNull String getFormatting() {
        ...
        if (modifier.getColor() != null) {
            ...
            if (modifier.getColor().getLegacyColor() == EnumChatFormat.WHITE) {
                ...
                // preventing unwanted &r -> &f conversion and stopping the <1.13 client bug fix
                // from working
