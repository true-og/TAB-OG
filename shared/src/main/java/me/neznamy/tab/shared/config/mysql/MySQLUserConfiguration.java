...
public class MySQLUserConfiguration implements PropertyConfiguration {
    ...
    @Override
    public void setProperty(@NotNull String user, @NotNull String property, @Nullable String server,
        ...
        try {
            ...
            if (getProperty(lowercaseUser, property, server, world) != null) {
                ...
                mysql.execute(
                        ...
                        lowercaseUser, property, world, server);

            }

            if (p != null)
                setProperty0(p, property, server, world, value);
            if (value != null)
                mysql.execute(
                        "insert into `tab_users` (`user`, `property`, `value`, `world`, `server`) values (?, ?, ?, ?, ?)",
                        lowercaseUser, property, value, world, server);
    ...
    private void setProperty0(@NotNull TabPlayer user, @NotNull String property, @Nullable String server,
            @Nullable String world, @Nullable String value)
    {

        if (world != null) {

            perWorld.computeIfAbsent(world, w -> new WeakHashMap<>()).computeIfAbsent(user, g -> new HashMap<>())
                    .put(property, value);

        } else if (server != null) {

            perServer.computeIfAbsent(server, s -> new WeakHashMap<>()).computeIfAbsent(user, g -> new HashMap<>())
                    .put(property, value);
    ...
    @Override
    public String[] getProperty(@NotNull String user, @NotNull String property, @Nullable String server,
            @Nullable String world)
    {

        TabPlayer p = getPlayer(user);
        Object value;
        if ((value = perWorld.getOrDefault(world, new WeakHashMap<>()).getOrDefault(p, new HashMap<>())
                .get(property)) != null)
        {
        ...
        if ((value = perServer.getOrDefault(server, new WeakHashMap<>()).getOrDefault(p, new HashMap<>())
                .get(property)) != null)
        {
    ...
    public void load(@NotNull TabPlayer player) {
        ...
        TAB.getInstance().getCPUManager().runTask(() -> {
            ...
            try {
                ...
                CachedRowSet crs = mysql.getCRS("select * from `tab_users` where `user` = ?",
                ...
                while (crs.next()) {
                    ...
                    TAB.getInstance().debug("Loaded user line: "
                            + String.format("%s, %s, %s, %s, %s", user, property, value, world, server));
                    setProperty0(player, property, server, world, value);

                }

                CachedRowSet crs2 = mysql.getCRS("select * from `tab_users` where `user` = ?",
