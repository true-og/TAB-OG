...
public class PlayerSlot {
    ...
    public void setPlayer(@Nullable TabPlayer newPlayer) {
        ...
        if (player != null)
            text = "";
        if (layout.getViewer().getVersion().getMinorVersion() < 8)
            return;
    ...
    public @NotNull TabList.Entry getSlot(@NotNull TabPlayer p) {
        ...
        if (player != null) {
            ...
            data = new TabList.Entry(uniqueId, layout.getManager().getDirection().getEntryName(p, slot),
                    player.getSkin(), true, player.getPing(), 0,
                    playerList == null ? new SimpleComponent(player.getName()) : playerList.getTabFormat(player, p));

        } else {

            data = new TabList.Entry(uniqueId, layout.getManager().getDirection().getEntryName(p, slot),
                    layout.getManager().getSkinManager().getDefaultSkin(slot), true,
                    layout.getManager().getEmptySlotPing(), 0, new SimpleComponent(text));
    ...
    public void setText(@NotNull String text) {
        ...
        if (this.text.equals(text) && player == null)
            return;
        this.text = text;
        if (player != null) {
