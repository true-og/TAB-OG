...
/**
 * A dynamic text with placeholder support. If any placeholder used in this text
 * changes value, feature defining this text will receive refresh call letting
 * it know and allowing to get new value.
 */
public class Property {
    ...
    /**
     * Internal identifier for this text for PlaceholderAPI expansion, null if it
    ...
    /**
     * Feature defining this text, which will receive refresh function if any of
     * placeholders used in it change value.
     */
    @Nullable
    ...
    /**
     * Raw value using %s for each placeholder ready to be inserted into String
     * formatter, which results in about 5x lower memory allocations as well as
     * better performance.
     */
    ...
    /**
     * All placeholders used in the text in the same order they are used, it may
     * contain duplicates if placeholder is used more than once. Contains relational
     * placeholders as well, which will get formatted to their identifier.
     */
    ...
    /**
     * Constructs new instance with given parameters and prepares the formatter for
     * use by detecting placeholders and reformatting the text.
     *
     * @param name     Property name to use in expansion (nullable if not use)
     * @param listener Feature which should receive refresh method if placeholder
     *                 changes value
    ...
    /**
     * Finds all placeholders used in the value and prepares it for String formatter
     * using %s for each placeholder.
     *
    ...
    private void analyze(@NotNull String value) {
        ...
        // Apply gradients that do not include placeholders to avoid applying them on
        // every refresh
    ...
    /**
     * Changes raw value to new provided value and performs all operations related
     * to it. Changes source as well.
     *
    ...
    public void changeRawValue(@NotNull String newValue, @Nullable String newSource) {
        ...
        if (originalRawValue.equals(newValue))
            return;
    ...
    /**
     * Replaces all placeholders in current raw value, colorizes it and returns
    ...
    public boolean update() {
        ...
        if ("%s".equals(rawFormattedValue)) {
            ...
            string = TAB.getInstance().getPlaceholderManager().getPlaceholder(placeholders[0]).set(placeholders[0],
                    owner);
            ...
            for (int i = 0; i < placeholders.length; i++) {
                ...
                values[i] = TAB.getInstance().getPlaceholderManager().getPlaceholder(placeholders[i])
        ...
        if (!lastReplacedValue.equals(string)) {
            ...
            if (name != null) {
                ...
                TAB.getInstance().getPlaceholderManager().getTabExpansion().setPropertyValue(owner, name,
                        lastReplacedValue);
    ...
    /**
     * Returns value for defined viewer by applying relational placeholders to last
     * known value
     *
    ...
    public @NotNull String getFormat(@NotNull TabPlayer viewer) {
        ...
        for (String identifier : relPlaceholders) {
            ...
            RelationalPlaceholderImpl pl = (RelationalPlaceholderImpl) TAB.getInstance().getPlaceholderManager()
        ...
        for (String identifier : TAB.getInstance().getPlaceholderManager().detectPlaceholders(format)) {
            ...
            format = format.replace(pl.getIdentifier(), pl.getLastValue(viewer, owner));
            if (listener != null)
                listener.addUsedPlaceholder(identifier);
