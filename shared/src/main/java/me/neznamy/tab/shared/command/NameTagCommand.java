...
public class NameTagCommand extends SubCommand {
    ...
    public void execute(@Nullable TabPlayer sender, @NotNull String[] args) {
        ...
        switch (args[0].toLowerCase(Locale.US)) {
            ...
            case "preview":
                preview(sender, getTarget(sender, args, TabConstants.Permission.COMMAND_NAMETAG_PREVIEW_OTHER,
                        TabConstants.Permission.COMMAND_NAMETAG_PREVIEW), silent);
                break;
            case "toggle":
                toggle(sender, getTarget(sender, args, TabConstants.Permission.COMMAND_NAMETAG_TOGGLE_OTHER,
                        TabConstants.Permission.COMMAND_NAMETAG_TOGGLE), silent);
                break;
            default:
                sendMessages(sender, getMessages().getNameTagHelpMenu());
                break;
    ...
    private @Nullable TabPlayer getTarget(@Nullable TabPlayer sender, @NotNull String[] args,
            @NotNull String permissionOther, @NotNull String permission)
    {
    ...
    public @NotNull List<String> complete(@Nullable TabPlayer sender, @NotNull String[] arguments) {
        ...
        if (arguments.length == 1)
            return getStartingArgument(Arrays.asList("toggle", "preview"), arguments[0]);
        if (arguments.length == 2)
            return getOnlinePlayers(arguments[1]);
