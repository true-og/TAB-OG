...
public class BukkitPlatform implements BackendPlatform {
    ...
    public void registerUnknownPlaceholder(@NotNull String identifier) {
        ...
        if (identifier.startsWith("%rel_")) {
            ...
            boolean stripLeadingAsterisk = identifier.startsWith("%server_tps");
            TAB.getInstance().getPlaceholderManager().registerServerPlaceholder(identifier,
                    () -> {
                        String out = PlaceholderAPI.setPlaceholders(null, identifier);
                        return stripLeadingAsterisk && out != null && out.startsWith("*") ? out.substring(1) : out;
                    });
        } else {
