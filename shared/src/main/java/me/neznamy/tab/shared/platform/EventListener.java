 ...
 *
 * @param <T> Platform's player class
 */
public interface EventListener<T> {
     ...
     *
     * @param player UUID of player who left
     */
    default void quit(@NotNull UUID player) {
        ...
        if (TAB.getInstance().isPluginDisabled())
            return;
        TAB.getInstance().getCPUManager()
                .runTask(() -> TAB.getInstance().getFeatureManager().onQuit(TAB.getInstance().getPlayer(player)));
     ...
     *
     * @param player Player who switched world
     ...
     *
     * @param player  UUID of player who received message
    ...
    /**
     * Replaces player object reference. Needed on modded servers, as vanilla
     * replaces player object on respawn, and they preserve that behavior.
     *
     * @param player    UUID of affected player
    ...
    /**
     * Forwards command preprocess to all features. Returns {@code true} if the
     * event should be cancelled, {@code false} if not.
     *
     * @param player  Player who ran the command
     * @param command Executed command including /
     ...
     *
     * @param player Platform's player object
