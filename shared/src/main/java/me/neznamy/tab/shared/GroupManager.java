...
public class GroupManager {
     ...
     */
    private final boolean groupsByPermissions = TAB.getInstance().getConfiguration().getConfig()
            .getBoolean("assign-groups-by-permissions", false);

    /**
     * List of group permissions to iterate through if {@link #groupsByPermissions}
     * is {@code true}
     */
    private final List<String> primaryGroupFindingList = TAB.getInstance().getConfiguration().getConfig()
            .getStringList("primary-group-finding-list", Arrays.asList("Owner", "Admin", "Helper", "default"));

    /**
     * Constructs new instance with given permission plugin and registers group
     * placeholder.
     *
     * @param permissionPlugin Name of detected permission plugin
     * @param groupFunction    Function returning group of a player
     */
    public GroupManager(@NotNull String permissionPlugin, @NotNull Function<TabPlayer, String> groupFunction) {
        ...
        TAB.getInstance().getCPUManager().startRepeatingMeasuredTask(
                TAB.getInstance().getConfiguration().getPermissionRefreshInterval(), "Permission group refreshing",
                "Refreshing task", () ->
                {
     ...
     *
     * @param player Player to detect permission group of
     ...
     *
     * @param player Player to get permission group of
    ...
    /**
     * Returns player's permission group based on highest permission or
     * {@link TabConstants#NO_GROUP} if player has no permission.
     *
     * @param player Player to get permission group of
     * @return Highest permission group player has permission for or
     *         {@link TabConstants#NO_GROUP} if player does not have any
     */
