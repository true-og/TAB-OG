...
public class PacketTabList1193 extends PacketTabList18 {
    ...
    public static void loadNew() throws ReflectiveOperationException {
        ...
        Class<?> RemoteChatSession$Data = BukkitReflection.getClass("network.chat.RemoteChatSession$Data",
        ...
        newRemovePacket = BukkitReflection.getClass("network.protocol.game.ClientboundPlayerInfoRemovePacket")
        ...
        actionToEnumSet.put(Action.UPDATE_GAME_MODE,
                EnumSet.of(Enum.valueOf(ActionClass, Action.UPDATE_GAME_MODE.name())));
            ...
            boolean listed, int latency, int gameMode, @Nullable Object displayName)
    {
        ...
        PLAYERS.set(packet,
                Collections.singletonList(newPlayerInfoData.newInstance(id,
                        action == Action.ADD_PLAYER ? createProfile(id, name, skin) : null, listed, latency,
                        gameModes[gameMode], displayName, null)));
        return packet;
    ...
    public void onPacketSend(@NonNull Object packet) {
        ...
        for (Object nmsData : (List<?>) PLAYERS.get(packet)) {
            ...
            // 1.19.3 is using records, which do not allow changing final fields, need to
            // rewrite the list entirely
            updatedList.add(rewriteEntry ? newPlayerInfoData.newInstance(id, profile,
                    PlayerInfoData_Listed.getBoolean(nmsData), latency, PlayerInfoData_GameMode.get(nmsData),
                    displayName, PlayerInfoData_RemoteChatSession.get(nmsData)) : nmsData);
