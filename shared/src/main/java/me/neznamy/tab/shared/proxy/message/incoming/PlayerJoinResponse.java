...
public class PlayerJoinResponse implements IncomingMessage {
    ...
    public void process(@NotNull ProxyTabPlayer player) {
        ...
        TAB.getInstance().debug("Bridge took " + (System.currentTimeMillis() - player.getBridgeRequestTime())
        ...
        // reset attributes from previous server to default false values, new server
        // will send separate update packets
        // if needed
        if (player.vanished) { // Only trigger if bridge says player is vanished, do not trigger on proxy
                               // vanish
