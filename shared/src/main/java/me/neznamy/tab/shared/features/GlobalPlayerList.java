 ...
 */
public class GlobalPlayerList extends TabFeature implements JoinListener, QuitListener, VanishListener,
        GameModeListener, Loadable, UnLoadable, ServerSwitchListener, TabListClearListener, Refreshable
{

    // config options
    private final List<String> spyServers = config()
            .getStringList("global-playerlist.spy-servers", Collections.singletonList("spyserver1")).stream()
            .map(String::toLowerCase).collect(Collectors.toList());
    private final Map<String, List<String>> sharedServers = config()
            .getConfigurationSection("global-playerlist.server-groups");
    private final boolean othersAsSpectators = config().getBoolean("global-playerlist.display-others-as-spectators",
            false);
    private final boolean vanishedAsSpectators = config()
            .getBoolean("global-playerlist.display-vanished-players-as-spectators", true);
    private final boolean isolateUnlistedServers = config().getBoolean("global-playerlist.isolate-unlisted-servers",
            false);
    private final Map<String, String> serverToGroup = new HashMap<>();
    private final PlayerList playerlist = TAB.getInstance().getFeatureManager()
    ...
    public GlobalPlayerList() {
        ...
        for (Map.Entry<String, List<String>> entry : sharedServers.entrySet()) {
            ...
            TAB.getInstance().getPlaceholderManager().registerServerPlaceholder(
                    TabConstants.Placeholder.globalPlayerListGroup(entry.getKey()), 1000, () ->
                    {

                        int count = 0;
                        for (TabPlayer player : TAB.getInstance().getOnlinePlayers()) {

                            if (entry.getValue().contains(player.getServer()) && !player.isVanished())
                                count++;
    ...
    public void load() {
        ...
        for (TabPlayer viewer : TAB.getInstance().getOnlinePlayers()) {
            ...
            for (TabPlayer displayed : TAB.getInstance().getOnlinePlayers()) {
                ...
                if (viewer.getServer().equals(displayed.getServer()))
                    continue;
                if (shouldSee(viewer, displayed))
                    entries.add(getAddInfoData(displayed, viewer));
    ...
    /**
     * Returns {@code true} if viewer should see the target player, {@code false} if
     * not.
     *
    ...
    public boolean shouldSee(@NotNull TabPlayer viewer, @NotNull TabPlayer displayed) {
        ...
        if (!TAB.getInstance().getPlatform().canSee(viewer, displayed))
            return false;
        if (isSpyServer(viewer.getServer()))
            return true;
    ...
    /**
     * Returns server group of specified server. If not part of any group, returns
     * either default or unique name if isolate unlisted servers is enabled.
     *
                ...
                for (String serverDefinition : group.getValue()) {
                    ...
                    if (serverDefinition.endsWith("*")) {
                        ...
                        if (server.toLowerCase()
                                .startsWith(serverDefinition.substring(0, serverDefinition.length() - 1).toLowerCase()))
                            return group.getKey();
    ...
    public void unload() {
        ...
        for (TabPlayer displayed : TAB.getInstance().getOnlinePlayers()) {
            ...
            for (TabPlayer viewer : TAB.getInstance().getOnlinePlayers()) {
                ...
                if (!displayed.getServer().equals(viewer.getServer()))
    ...
    public void onJoin(@NotNull TabPlayer connectedPlayer) {
        ...
        for (TabPlayer all : TAB.getInstance().getOnlinePlayers()) {
            ...
            if (connectedPlayer.getServer().equals(all.getServer()))
                continue;
    ...
    public void onServerChange(@NotNull TabPlayer changed, @NotNull String from, @NotNull String to) {
        ...
        // Player who switched server is removed from tablist of other players in
        // ~70-110ms (depending on online count),
        // re-add with a delay
        TAB.getInstance().getCPUManager().runTaskLater(200, getFeatureName(),
                TabConstants.CpuUsageCategory.SERVER_SWITCH, () ->
                {
                    ...
                    for (TabPlayer all : TAB.getInstance().getOnlinePlayers()) {
                        ...
                        // Remove for everyone and add back if visible, easy solution to
    ...
    public TabList.Entry getAddInfoData(@NotNull TabPlayer p, @NotNull TabPlayer viewer) {
        ...
        return new TabList.Entry(p.getTablistId(), p.getNickname(), p.getSkin(), true, p.getPing(), gameMode,
    ...
    public void onGameModeChange(@NotNull TabPlayer player) {
        ...
        for (TabPlayer viewer : TAB.getInstance().getOnlinePlayers()) {
            ...
            if (!player.getServer().equals(viewer.getServer())) {
                ...
                viewer.getTabList().updateGameMode(player.getTablistId(),
                        othersAsSpectators ? 3 : player.getGamemode());
    ...
    /**
     * Returns {@code true} if specified server is spy-server, {@code false} if not.
     *
    ...
    public void refresh(@NotNull TabPlayer refreshed, boolean force) {
        ...
        // player ping changed, must manually update latency for players on other
        // servers
        for (TabPlayer viewer : TAB.getInstance().getOnlinePlayers()) {

            if (!refreshed.getServer().equals(viewer.getServer())
                    && viewer.getTabList().containsEntry(refreshed.getTablistId()))
            {
