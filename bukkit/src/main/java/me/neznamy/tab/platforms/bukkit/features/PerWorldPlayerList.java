...
public class PerWorldPlayerList extends TabFeature implements Listener, Loadable, UnLoadable {
    ...
    private final List<String> ignoredWorlds = config().getStringList("per-world-playerlist.ignore-effect-in-worlds",
            Arrays.asList("ignored_world", "build"));
    private final Map<String, List<String>> sharedWorlds = config()
            .getConfigurationSection("per-world-playerlist.shared-playerlist-world-groups");
    ...
    public void onJoin(PlayerJoinEvent e) {
        ...
        TAB.getInstance().getCPUManager().addTime(getFeatureName(), TabConstants.CpuUsageCategory.PLAYER_JOIN,
    ...
    public void onWorldChange(PlayerChangedWorldEvent e) {
        ...
        long time = System.nanoTime();
        checkPlayer(e.getPlayer());
        TAB.getInstance().getCPUManager().addTime(getFeatureName(), TabConstants.CpuUsageCategory.WORLD_SWITCH,
                System.nanoTime() - time);

    }

    /**
     * Performs visibility check on the player. Shows players this player should see
     * and does not, hides players the player should not see, shows the player to
     * those who should see player and hides from those, who should not see.
     *
    ...
    /**
     * Returns {@code true} if viewer should see target player, {@code false} if
     * not.
     * 
     * @param viewer Player viewing the TabList
     * @param target Target displayed in the TabList
    ...
    private boolean shouldSee(@NotNull Player viewer, @NotNull Player target) {
        ...
        if ((allowBypass && viewer.hasPermission(TabConstants.Permission.PER_WORLD_PLAYERLIST_BYPASS))
                || ignoredWorlds.contains(viewer.getWorld().getName()))
            return true;
        String viewerWorldGroup = viewer.getWorld().getName() + "-default"; // preventing unwanted behavior when some
                                                                            // group is called exactly like a world
