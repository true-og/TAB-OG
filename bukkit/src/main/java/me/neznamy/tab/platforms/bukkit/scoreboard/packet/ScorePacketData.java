...
public class ScorePacketData {
    ...
    /**
     * Constructs new instance and loads all fields. Throws exception if something
     * went wrong.
     *
     * @throws ReflectiveOperationException If something went wrong
     */
    public ScorePacketData() throws ReflectiveOperationException {

        Class<?> SetScorePacket = BukkitReflection.getClass("network.protocol.game.ClientboundSetScorePacket", // Mojang
                                                                                                               // mapped
        ...
        if (BukkitReflection.is1_20_3Plus()) {
            ...
            try {
                ...
                // 1.20.5+
                Constructor<?> newSetScore = SetScorePacket.getConstructor(String.class, String.class, int.class,
                        Optional.class, Optional.class);
                setScore0 = (objective, holder, score, displayName, numberFormat) -> newSetScore.newInstance(holder,
                        objective, score, Optional.ofNullable(displayName), Optional.ofNullable(numberFormat));

            } catch (ReflectiveOperationException e) {

                // 1.20.3 - 1.20.4
                Constructor<?> newSetScore = SetScorePacket.getConstructor(String.class, String.class, int.class,
                        PacketScoreboard.Component, PacketScoreboard.NumberFormat);
                setScore0 = (objective, holder, score, displayName, numberFormat) -> newSetScore.newInstance(holder,
                        objective, score, displayName, numberFormat);

            }

            setScore = setScore0;
            Constructor<?> newResetScore = BukkitReflection
                    .getClass("network.protocol.game.ClientboundResetScorePacket")
            ...
            Class<?> actionClass = BukkitReflection.getClass("server.ServerScoreboard$Method",
                    "server.ScoreboardServer$Action", "ScoreboardServer$Action");
            Constructor<?> newSetScore = SetScorePacket.getConstructor(actionClass, String.class, String.class,
                    int.class);
            Enum<?>[] scoreboardActions = (Enum<?>[]) actionClass.getMethod("values").invoke(null);
            setScore = (objective, holder, score, displayName, numberFormat) -> newSetScore
                    .newInstance(scoreboardActions[0], objective, holder, score);
            ...
            Constructor<?> newScoreboardScore = ScoreboardScore.getConstructor(PacketScoreboard.Scoreboard,
                    PacketScoreboard.ScoreboardObjective, String.class);
            ...
            setScore = (objective, holder, score, displayName, numberFormat) -> {
                ...
                Object scoreboardScore = newScoreboardScore.newInstance(PacketScoreboard.emptyScoreboard,
                        PacketScoreboard.newScoreboardObjective.newInstance(PacketScoreboard.emptyScoreboard, objective,
                                PacketScoreboard.IScoreboardCriteria_dummy),
                        holder);
                Object packet;
                if (BukkitReflection.getMinorVersion() >= 8) {
     ...
     * @param displayName  Score holder display name
     * @param numberFormat Score value number format
