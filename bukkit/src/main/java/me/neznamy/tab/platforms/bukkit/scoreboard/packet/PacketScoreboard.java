...
/**
 * Scoreboard implementation which uses packets to send scoreboards to use the
 * full potential on all versions and server software without any artificial
 * limits.
 */
public class PacketScoreboard extends Scoreboard<BukkitTabPlayer, Object> {
    ...
    static {
        ...
        try {
            ...
            ScoreboardObjective = BukkitReflection.getClass("world.scores.Objective",
                    "world.scores.ScoreboardObjective", "ScoreboardObjective");
            Class<?> IScoreboardCriteria = BukkitReflection.getClass("world.scores.criteria.ObjectiveCriteria", // Mojang
                                                                                                                // mapped
                    "world.scores.criteria.IScoreboardCriteria", // Bukkit 1.17.+
                    "IScoreboardCriteria", // 1.5.1 - 1.16.5
                    "IObjective" // 1.5.0
            );
            ObjectivePacketClass = BukkitReflection.getClass("network.protocol.game.ClientboundSetObjectivePacket", // Mojang
                                                                                                                    // mapped
            ...
            IScoreboardCriteria_dummy = ReflectionUtils.getFields(IScoreboardCriteria, IScoreboardCriteria).get(0)
                    .get(null);
            ...
            if (minorVersion >= 8) {
                ...
                if (minorVersion < 13) {
                    ...
                    Objective_RENDER_TYPE = ReflectionUtils.getOnlyField(ObjectivePacketClass,
            ...
            if (minorVersion < 13) {
                ...
                ScoreboardObjective_setDisplayName = ReflectionUtils.getOnlyMethod(ScoreboardObjective, void.class,
                        String.class);
            ...
            if (BukkitReflection.is1_20_3Plus()) {
                ...
                newFixedFormat = BukkitReflection.getClass("network.chat.numbers.FixedFormat")
    ...
    public void setDisplaySlot0(int slot, @NonNull String objective) {
        ...
        packetSender.sendPacket(player.getPlayer(),
                displayPacketData.setDisplaySlot(slot, newObjective(objective, "", 0, null)));
            ...
            @Nullable Object numberFormat)
    {
        ...
        packetSender.sendPacket(player.getPlayer(),
                newObjectivePacket(ObjectiveAction.REGISTER, objectiveName, title, display, numberFormat));
    ...
    public void unregisterObjective0(@NonNull String objectiveName) {
        ...
        packetSender.sendPacket(player.getPlayer(),
                newObjectivePacket(ObjectiveAction.UNREGISTER, objectiveName, "", 0, null));
            ...
            @Nullable Object numberFormat)
    {
        ...
        packetSender.sendPacket(player.getPlayer(),
                newObjectivePacket(ObjectiveAction.UPDATE, objectiveName, title, display, numberFormat));
            ...
            @Nullable Object numberFormat)
    {
        ...
        Object packet = newObjectivePacket.newInstance(newObjective(objectiveName, title, display, numberFormat),
                action);
            ...
            int options, @NonNull EnumChatFormat color)
    {
        ...
        teams.put(name, team);
        packetSender.sendPacket(player.getPlayer(), teamPacketData.registerTeam(team, prefix, toComponent(prefix),
                suffix, toComponent(suffix), visibility, collision, players, options, color));
            ...
            @NonNull EnumChatFormat color)
    {
        ...
        packetSender.sendPacket(player.getPlayer(), teamPacketData.updateTeam(teams.get(name), prefix,
                toComponent(prefix), suffix, toComponent(suffix), visibility, collision, options, color));
            ...
            @Nullable Object displayName, @Nullable Object numberFormat)
    {
        ...
        packetSender.sendPacket(player.getPlayer(),
                scorePacketData.setScore(objective, scoreHolder, score, displayName, toFixedFormat(numberFormat)));
    ...
    public void onPacketSend(@NonNull Object packet) {
        ...
        if (ObjectivePacketClass.isInstance(packet)) {
            ...
            TAB.getInstance().getFeatureManager().onObjective(player, Objective_METHOD.getInt(packet),
                    (String) Objective_OBJECTIVE_NAME.get(packet));
     ...
     * @param numberFormat  Default number format (1.20.3+)
            ...
            @Nullable Object numberFormat)
    {
        ...
        if (BukkitReflection.is1_20_3Plus()) {
            ...
            // 1.20.3+
            return newScoreboardObjective.newInstance(emptyScoreboard, objectiveName, null, // Criteria
                    toComponent(title), healthDisplays[renderType], false, // Auto update
        ...
        if (BukkitReflection.getMinorVersion() >= 13) {
            ...
            // 1.13 - 1.20.2
            return newScoreboardObjective.newInstance(emptyScoreboard, objectiveName, null, // Criteria
                    toComponent(title), healthDisplays[renderType]);

        }

        // 1.5 - 1.12.2
        Object objective = newScoreboardObjective.newInstance(emptyScoreboard, objectiveName,
                IScoreboardCriteria_dummy);
    ...
    private Object toFixedFormat(@Nullable Object numberFormat) {
        ...
        if (numberFormat == null || newFixedFormat == null)
            return null;
