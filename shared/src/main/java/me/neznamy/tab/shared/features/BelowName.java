 ...
 */
public class BelowName extends TabFeature implements JoinListener, Loadable, UnLoadable, Refreshable,
        LoginPacketListener, WorldSwitchListener, ServerSwitchListener
{
    ...
    private final String fancyDisplayPlayers = config().getString("belowname-objective.fancy-display-players",
            "&c" + TabConstants.Placeholder.HEALTH);
    ...
    /**
     * Constructs new instance and registers disable condition checker and text
     * refresher to feature manager.
     */
    public BelowName() {

        Condition disableCondition = Condition
                .getCondition(config().getString("belowname-objective.disable-condition"));
        disableChecker = new DisableChecker(getFeatureName(), disableCondition, this::onDisableConditionChange,
                p -> p.disabledBelowname);
        TAB.getInstance().getFeatureManager().registerFeature(TabConstants.Feature.BELOW_NAME + "-Condition",
                disableChecker);
    ...
    public void load() {
        ...
        for (TabPlayer viewer : TAB.getInstance().getOnlinePlayers()) {
            ...
            for (Map.Entry<TabPlayer, Integer> entry : values.entrySet()) {
                ...
                if (!sameServerAndWorld(viewer, target))
                    continue;
    ...
    public void onJoin(@NotNull TabPlayer connectedPlayer) {
        ...
        for (TabPlayer all : TAB.getInstance().getOnlinePlayers()) {
            ...
            if (!sameServerAndWorld(connectedPlayer, all))
                continue;
            setScore(all, connectedPlayer, number, fancy.getFormat(all));
            if (all != connectedPlayer) {

                setScore(connectedPlayer, all, getValue(all),
                        all.getProperty(FANCY_FORMAT_PROPERTY).getFormat(connectedPlayer));
        ...
        if (redis != null)
            redis.updateBelowName(connectedPlayer, number, fancy.get());
    ...
    public void refresh(@NotNull TabPlayer refreshed, boolean force) {
        ...
        for (TabPlayer viewer : TAB.getInstance().getOnlinePlayers()) {
            ...
            if (!sameServerAndWorld(viewer, refreshed))
                continue;
        ...
        if (redis != null)
            redis.updateBelowName(refreshed, number, fancy.get());
    ...
    public void onLoginPacket(@NotNull TabPlayer player) {
        ...
        if (player.disabledBelowname.get() || !player.isLoaded())
            return;
        register(player);
        for (TabPlayer all : TAB.getInstance().getOnlinePlayers()) {
    ...
    private void register(@NotNull TabPlayer player) {
        ...
        player.getScoreboard().registerObjective(OBJECTIVE_NAME, player.getProperty(TEXT_PROPERTY).updateAndGet(),
     ...
     * @param fancyDisplay NumberFormat display of the score
     */
    public void setScore(@NotNull TabPlayer viewer, @NotNull TabPlayer scoreHolder, int value,
            @NotNull String fancyDisplay)
    {

        if (viewer.disabledBelowname.get())
            return;
        viewer.getScoreboard().setScore(OBJECTIVE_NAME, scoreHolder.getNickname(), value, null, // Unused by this
                                                                                                // objective slot
     ...
     * @return {@code true} if players are in the same server and world,
    ...
    private void updatePlayer(@NotNull TabPlayer player) {
        ...
        for (TabPlayer all : TAB.getInstance().getOnlinePlayers()) {
            ...
            setScore(player, all, getValue(all), all.getProperty(FANCY_FORMAT_PROPERTY).getFormat(player));
            if (all != player)
                setScore(all, player, getValue(player), player.getProperty(FANCY_FORMAT_PROPERTY).getFormat(all));
    ...
    private static class TextRefresher extends TabFeature implements Refreshable {
        ...
        public void refresh(@NotNull TabPlayer refreshed, boolean force) {
            ...
            if (refreshed.disabledBelowname.get())
                return;
            refreshed.getScoreboard().updateObjective(OBJECTIVE_NAME,
                    refreshed.getProperty(feature.TEXT_PROPERTY).updateAndGet(), Scoreboard.HealthDisplay.INTEGER,
