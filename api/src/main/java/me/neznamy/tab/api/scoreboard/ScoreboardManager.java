...
/**
 * Interface allowing work with scoreboards, such as creating, sending, toggling
 * and modifying.
 * <p>
 * Instance can be obtained using
 * {@link me.neznamy.tab.api.TabAPI#getScoreboardManager()}. This requires the
 * scoreboard feature to be enabled in config, the method will return
 * {@code null} otherwise.
 */
...
public interface ScoreboardManager {
    ...
    /**
     * Creates new scoreboard, registers and returns it. {@code name} is internal
     * name of the scoreboard, which it will be registered with and further work
     * where scoreboard name is used allowed.
     * <p>
     ...
     * <p>
     * {@code lines} will display in the same order as contained in the list. Client
     * can only see up to 15 lines at a time, however you can define more, if some
     * of them only consist of placeholder that might be empty.
     *
     * @param name  Internal name of the scoreboard
     * @param title The scoreboard title
     * @param lines Lines of text in scoreboard (supports placeholders)
     * @return The new scoreboard with given parameters
     * @throws NullPointerException if {@code name} or {@code lines} is {@code null}
     */
    @NotNull
    Scoreboard createScoreboard(@NonNull String name, @NonNull String title, @NonNull List<String> lines);

    /**
     * Returns map of registered scoreboards via config and API. Map key is name of
     * scoreboard, which is defined either in config or using
     * {@link #createScoreboard(String, String, List)}. Map value is reference to
     ...
     * {@link #resetScoreboard(TabPlayer)}. If the player already sees this
     * scoreboard, nothing will happen.
     *
     * @param player     Player to send scoreboard to
     * @param scoreboard Scoreboard to display
     * @throws NullPointerException if {@code player} or {@code scoreboard} is
     *                              {@code null}
    ...
    /**
     * Hides custom scoreboard sent using
     * {@link #showScoreboard(TabPlayer, Scoreboard)} and re-enables internal
     * display logic with conditions. If player does not have any forced scoreboard,
     * nothing happens.
     *
    ...
    /**
     * Returns {@code true} if player has scoreboard enabled, {@code false} if
     * disabled (toggled) using either toggle command,
     ...
     * visibility, scoreboard is toggled. If {@code sendToggleMessage} is
     * {@code true}, toggle message defined in configuration is sent.
     *
     * @param player            Player to set visibility for
     * @param visible           New visibility status
     * @param sendToggleMessage whether to send toggle message if status changed or
     *                          not
     * @see #toggleScoreboard(TabPlayer, boolean)
     ...
     * (in milliseconds). Scoreboard name is either defined in config, or through
     * API in {@link #createScoreboard(String, String, List)}.
     *
     * @param scoreboard Scoreboard from config or registered via API
     * @param duration   Duration in milliseconds
     * @throws IllegalArgumentException if no scoreboard was found with such name or
     *                                  {@code duration} is &lt; 0.
     */
     ...
     * if player does not see any scoreboard due to not meeting any display
     * condition.
     *
     * @param player player to get active scoreboard of
     * @return player's active scoreboard or {@code null} if player has no
     *         scoreboard
     */
    @Nullable
