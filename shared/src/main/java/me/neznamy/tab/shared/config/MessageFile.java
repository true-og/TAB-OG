...
public class MessageFile extends YamlConfigurationFile {
    ...
    private final String announceCommandUsage = getString(
    ...
    private final String bossBarNotEnabled =
            getString("bossbar-feature-not-enabled", "&cThis command requires the bossbar feature to be enabled.");
    private final String bossBarAnnounceCommandUsage =
            getString("bossbar-announce-command-usage", "Usage: /tab announce bar <bar name> <length>");
    private final String bossBarAlreadyAnnounced =
            getString("bossbar-already-announced", "&cThis bossbar is already being announced");
    private final String parseCommandUsage =
            getString("parse-command-usage", "Usage: /tab parse <player> <placeholder>");
    private final String sendCommandUsage = getString(
            "send-command-usage",
            "Usage: /tab send <type> <player> <bar name> <length>\nCurrently supported types: &lbar");
    private final String sendBarCommandUsage =
            getString("send-bar-command-usage", "Usage: /tab send bar <player> <bar name> <length>");
    private final String teamFeatureRequired =
            getString("team-feature-required", "This command requires scoreboard teams feature enabled");
    private final String collisionCommandUsage =
            getString("collision-command-usage", "Usage: /tab setcollision <player> <true/false>");
    private final String noPermission = getString(
            "no-permission",
    ...
    private final String commandOnlyFromGame =
            getString("command-only-from-game", "&cThis command must be ran from the game");
    private final String unlimitedNametagModeNotEnabled = getString(
            "unlimited-nametag-mode-not-enabled",
            "&c[TAB] Warning! To make this feature work, you need to enable unlimited-nametag-mode in the config!");
    private final String scoreboardFeatureNotEnabled = getString(
            "scoreboard-feature-not-enabled", "&4This command requires the scoreboard feature to be enabled.");
    private final String scoreboardAnnounceCommandUsage = getString(
    ...
    private final String reloadFailBrokenFile = getString(
            "reload-fail-file",
    ...
    private final String scoreboardShowUsage =
            getString("scoreboard-show-usage", "Usage: /tab scoreboard show <scoreboard> [player]");
    private final String bossBarNotMarkedAsAnnouncement = getString(
    ...
    private final List<String> helpMenu = getStringList(
            "help-menu",
            Arrays.asList(
                    "&m                                                                                ",
                    " &8>> &3&l/tab reload",
                    ...
                    "&m                                                                                "));
    private final List<String> mySQLHelpMenu = getStringList(
            "mysql-help-menu",
            Arrays.asList(
    ...
    private final String mySQLFailNotEnabled =
            getString("mysql-fail-not-enabled", "&cCannot download/upload data from/to MySQL, because it's disabled.");
    private final String mySQLFailError =
            getString("mysql-fail-error", "MySQL download failed due to an error. Check console for more info.");
    private final String mySQLDownloadSuccess =
            getString("mysql-download-success", "&aMySQL data downloaded successfully.");
    private final String mySQLUploadSuccess = getString("mysql-upload-success", "&aMySQL data uploaded successfully.");
    private final List<String> nameTagHelpMenu = getStringList(
            "nametag-help-menu",
            Arrays.asList(
    ...
    private final String nameTagFeatureNotEnabled =
            getString("nametag-feature-not-enabled", "&cThis command requires nametag feature to be enabled.");
    ...
    private final String armorStandsDisabledCannotPreview = getString(
            "armorstands-disabled-cannot-use-preview",
            "&cYour armor stands are disabled, therefore you cannot use preview feature");
    private final List<String> scoreboardHelpMenu = getStringList(
    ...
    private final List<String> bossbarHelpMenu = getStringList(
            "bossbar-help-menu",
            Arrays.asList(
    ...
    public MessageFile() throws IOException {
        super(
                MessageFile.class.getClassLoader().getResourceAsStream("config/messages.yml"),
                new File(TAB.getInstance().getDataFolder(), "messages.yml"));
    }

    public @NotNull String getBossBarNotFound(@NotNull String name) {
        return getString("bossbar-not-found", "&cNo bossbar found with the name \"%name%\"")
                .replace("%name%", name);
    }
    ...
    public @NotNull String getGroupValueAssigned(
            @NotNull String property, @NotNull String value, @NotNull String group) {
        return getString(
                        "group-value-assigned",
    ...
    public @NotNull String getPlayerValueAssigned(
            @NotNull String property, @NotNull String value, @NotNull String player) {
        return getString(
                        "user-value-assigned",
    ...
    public @NotNull String getPlayerNotFound(@NotNull String name) {
        return getString("player-not-online", "&cNo online player found with the name \"%player%\"")
                .replace("%player%", name);
    }
    ...
    public @NotNull String getScoreboardNotFound(@NotNull String name) {
        return getString("scoreboard-not-found", "&cNo scoreboard found with the name \"%name%\"")
                .replace("%name%", name);
    }
