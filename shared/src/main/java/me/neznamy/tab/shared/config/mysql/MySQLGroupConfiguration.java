...
public class MySQLGroupConfiguration implements PropertyConfiguration {
    ...
    public MySQLGroupConfiguration(@NotNull MySQL mysql) throws SQLException {
        ...
        while (crs.next()) {
            ...
            if (!group.equals(TabConstants.DEFAULT_GROUP))
    ...
    @Override
    public void setProperty(@NotNull String group, @NotNull String property, @Nullable String server,
        ...
        try {
            ...
            if (getProperty(lowercaseGroup, property, server, world) != null) {
                ...
                mysql.execute(
                        "delete from `tab_groups` where `group` = ? and `property` = ? and world "
                                + querySymbol(world == null) + " ? and server " + querySymbol(server == null) + " ?",
                        lowercaseGroup, property, world, server);
            ...
            if (value != null)
                mysql.execute(
                        ...
                        lowercaseGroup, property, value, world, server);
    ...
    private void setProperty0(@NotNull String group, @NotNull String property, @Nullable String server,
            @Nullable String world, @Nullable String value)
    {

        if (world != null) {

            perWorld.computeIfAbsent(world, w -> new HashMap<>()).computeIfAbsent(group, g -> new HashMap<>())
                    .put(property, value);

        } else if (server != null) {

            perServer.computeIfAbsent(server, s -> new HashMap<>()).computeIfAbsent(group, g -> new HashMap<>())
                    .put(property, value);
    ...
    @Override
    public String[] getProperty(@NotNull String group, @NotNull String property, @Nullable String server,
            @Nullable String world)
    {

        String lowercaseGroup = group.equals(TabConstants.DEFAULT_GROUP) ? group : group.toLowerCase(Locale.US);
        Object value;
        if ((value = perWorld.getOrDefault(world, new HashMap<>()).getOrDefault(lowercaseGroup, new HashMap<>())
                .get(property)) != null)
        {
        ...
        if ((value = perWorld.getOrDefault(world, new HashMap<>())
                .getOrDefault(TabConstants.DEFAULT_GROUP, new HashMap<>()).get(property)) != null)
        {

            return new String[] { toString(value),
                    String.format("group=%s,world=%s", TabConstants.DEFAULT_GROUP, world) };

        }

        if ((value = perServer.getOrDefault(server, new HashMap<>()).getOrDefault(lowercaseGroup, new HashMap<>())
                .get(property)) != null)
        {
        ...
        if ((value = perServer.getOrDefault(server, new HashMap<>())
                .getOrDefault(TabConstants.DEFAULT_GROUP, new HashMap<>()).get(property)) != null)
        {

            return new String[] { toString(value),
                    String.format("group=%s,server=%s", TabConstants.DEFAULT_GROUP, server) };
    ...
    public void remove(@NotNull String group) {
        ...
        values.getOrDefault(group, new HashMap<>()).keySet()
                .forEach(property -> setProperty(group, property, null, null, null));
        perWorld.keySet().forEach(world -> perWorld.get(world).getOrDefault(group, new HashMap<>()).keySet()
                .forEach(property -> setProperty(group, property, null, world, null)));
        perServer.keySet().forEach(server -> perServer.get(server).getOrDefault(group, new HashMap<>()).keySet()
                .forEach(property -> setProperty(group, property, server, null, null)));
