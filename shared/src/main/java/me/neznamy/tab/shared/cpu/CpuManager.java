...
/**
 * A class which measures CPU usage of all tasks inserted into it and shows
 * usage
 */
public class CpuManager {
    ...
    /**
     * Scheduler for placeholder refreshing task to prevent inefficient placeholders
     * from lagging the entire plugin
     */
    @Getter
    ...
    /**
     * Enables CPU usage tracking and returns {@code true} if it was not enabled
     * previously. If it was, does nothing and returns {@code false}.
     *
     * @return {@code true} if this call enabled it, {@code false} if it was already
     *         enabled before
     */
    ...
    /**
     * Submits task to TAB's main thread. If plugin is not enabled yet, queues the
     * task instead and executes once it's loaded.
     *
     * @param task task to execute
     */
    private void submit(@NotNull Runnable task) {

        if (processingThread.isShutdown())
            return;
        if (!enabled) {
     ...
     *
     * @param times How long each placeholder took
     */
    ...
    public void startRepeatingMeasuredTask(int intervalMilliseconds, @NotNull String feature, @NotNull String type,
            @NotNull Runnable task)
    {

        if (processingThread.isShutdown())
            return;
        processingThread.scheduleAtFixedRate(() -> runAndMeasure(task, feature, type), intervalMilliseconds,
    ...
    public void startRepeatingTask(int intervalMilliseconds, @NotNull Runnable task) {
        ...
        if (processingThread.isShutdown())
            return;
        processingThread.scheduleAtFixedRate(() -> run(task), intervalMilliseconds, intervalMilliseconds,
                TimeUnit.MILLISECONDS);

    }

    public void runTaskLater(int delayMilliseconds, @NotNull String feature, @NotNull String type,
            @NotNull Runnable task)
    {
