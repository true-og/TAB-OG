 ...
 *
 * @param <P> Platform's player class
...
public abstract class TabList<P extends TabPlayer, C> {
    ...
    /**
     * Expected names based on configuration, saving to restore them if another
     * plugin overrides them
     */
    private final Map<TabPlayer, C> expectedDisplayNames = Collections.synchronizedMap(new WeakHashMap<>());

    private final RedisSupport redisSupport = TAB.getInstance().getFeatureManager()
    ...
    /**
     * Checks if all entries have display names as configured and if not, they are
     * forced. Only works on platforms with a full TabList API. Not needed for
     * platforms which support pipeline injection.
     */
     ...
     *
     * @param player Player with different display name than expected.
     * @param viewer Viewer of the TabList with wrong entry.
     */
    protected void displayNameWrong(@NonNull String player, @NonNull TabPlayer viewer) {

        TAB.getInstance().debug("TabList entry of player " + player + " has a different display name " + "for viewer "
    ...
    private void setExpectedDisplayName(@NonNull UUID entry, @Nullable C displayName) {
        ...
        if (player != null)
            expectedDisplayNames.put(player, displayName);

        if (redisSupport != null) {

            RedisPlayer redisPlayer = redisSupport.getRedisPlayers().get(entry);
            if (redisPlayer != null)
                expectedRedisDisplayNames.put(redisPlayer, displayName);
     ...
     *
     * @param id UUID of tablist entry
     ...
     * @param skin        Entry skin
     * @param listed      Whether entry should be listed or not
     * @param latency     Entry latency
     * @param gameMode    Entry game mode
     * @param displayName Entry display name
     */
    public abstract void addEntry0(@NonNull UUID id, @NonNull String name, @Nullable Skin skin, boolean listed,
            int latency, int gameMode, @Nullable C displayName);
    ...
    /**
     * Returns {@code true} if tablist contains specified entry, {@code false} if
     * not.
     *
     * @param entry UUID of entry to check
     * @return {@code true} if tablist contains specified entry, {@code false} if
     *         not
     */
    ...
    public static class Entry {
        ...
        /**
         * Display name displayed in TabList. Using {@code null} results in no display
         * name and scoreboard team prefix/suffix being visible in TabList instead.
         */
        @Nullable
