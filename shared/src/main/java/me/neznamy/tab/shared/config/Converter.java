...
public class Converter {
    ...
    public void convert2810to290(@NotNull ConfigurationFile animations) {
        ...
        if (animations.getValues().size() == 1 && animations.getValues().containsKey("animations")) {
            ...
            TAB.getInstance().getPlatform().logInfo(TabComponent.fromColoredText(
                    EnumChatFormat.YELLOW + "Performing configuration conversion from 2.8.10 to 2.9.0"));
    ...
    /**
     * Converts all configuration files from 2.9.2 into new 3.0.0 format and saves a
     * copy of old files into a new "old_configs" folder.
     *
     * @param currentConfig currently detected config.yml file
     * @throws IOException if an I/O operation with the files fails
     */
    public void convert292to300(@NotNull ConfigurationFile currentConfig) throws IOException {
        ...
        if (!currentConfig.hasConfigOption("change-nametag-prefix-suffix"))
            return;
        TAB.getInstance().getPlatform().logInfo(TabComponent
                .fromColoredText(EnumChatFormat.YELLOW + "Performing configuration conversion from 2.9.2 to 3.0.0"));
    ...
    /**
     * Creates a new "old_configs" folder and moves all files in TAB's folder into
     * it.
     *
     * @throws IOException if thrown by file move operation
     */
    private void moveOldFiles() throws IOException {
        ...
        for (File file : Objects.requireNonNull(folder.listFiles())) {
            ...
            if (!file.isFile())
                continue; // old_configs folder
            Files.move(file.toPath(),
                    new File(folder.getPath() + File.separator + "old_configs" + File.separator + file.getName())
                            .toPath(),
    ...
    private void convertTeamOptions(@NotNull ConfigurationFile oldConfig, @NotNull ConfigurationFile newConfig,
            @Nullable ConfigurationFile premiumConfig)
    {
        ...
        newConfig.set("scoreboard-teams.disable-in-worlds", oldConfig
                .getStringList("disable-features-in-worlds.nametag", Collections.singletonList("disabledworld")));
        if (TAB.getInstance().getPlatform().isProxy()) {

            newConfig.set("scoreboard-teams.disable-in-servers", oldConfig
                    .getStringList("disable-features-in-servers.nametag", Collections.singletonList("disabledserver")));

        } else {

            newConfig.set("scoreboard-teams.unlimited-nametag-mode.enabled",
                    oldConfig.getBoolean("unlimited-nametag-prefix-suffix-mode.enabled", false));
            newConfig.set("scoreboard-teams.unlimited-nametag-mode.disable-on-boats",
                    oldConfig.getBoolean("unlimited-nametag-prefix-suffix-mode.disable-on-boats", true));
            newConfig.set("scoreboard-teams.unlimited-nametag-mode.space-between-lines",
                    oldConfig.getBoolean("unlimited-nametag-prefix-suffix-mode.space-between-lines", true));
            newConfig.set("scoreboard-teams.unlimited-nametag-mode.disable-in-worlds", oldConfig.getStringList(
                    "disable-features-in-worlds.unlimited-nametags", Collections.singletonList("disabledworld")));
        ...
        if (premiumConfig != null) {
            ...
            newConfig.set("scoreboard-teams.case-sensitive-sorting",
                    premiumConfig.getBoolean("case-sensitive-sorting", true));
            newConfig.set("scoreboard-teams.unlimited-nametag-mode.dynamic-lines",
                    premiumConfig.getStringList("unlimited-nametag-mode-dynamic-lines",
                            Arrays.asList("abovename", "nametag", "belowname", "another")));
            newConfig.set("scoreboard-teams.unlimited-nametag-mode.static-lines",
                    premiumConfig.getConfigurationSection("unlimited-nametag-mode-static-lines"));
        ...
        for (String type : sortingType.split("_THEN_")) {
            ...
            if (type.equalsIgnoreCase("GROUPS") || type.equalsIgnoreCase("GROUP_PERMISSIONS")) {
                ...
                List<String> sortingList = oldConfig.getStringList("group-sorting-priority-list",
                        Arrays.asList("owner", "admin", "mod", "helper", "builder", "premium", "player", "default"));
                StringBuilder groups = new StringBuilder(
                        ("GROUP_PERMISSIONS".equals(type) ? "PERMISSIONS" : "GROUPS") + ":");
    ...
    private void convertTabListFormatting(@NotNull ConfigurationFile oldConfig, @NotNull ConfigurationFile newConfig) {
        ...
        newConfig.set("tablist-name-formatting.anti-override",
                oldConfig.getBoolean("anti-override.tablist-names", true));
        newConfig.set("tablist-name-formatting.disable-in-worlds", oldConfig
                .getStringList("disable-features-in-worlds.tablist-names", Collections.singletonList("disabledworld")));
        if (TAB.getInstance().getPlatform().isProxy())
            newConfig.set("tablist-name-formatting.disable-in-servers", oldConfig.getStringList(
                    "disable-features-in-servers.tablist-names", Collections.singletonList("disabledserver")));

    }

    private void convertYellowNumber(@NotNull ConfigurationFile oldConfig, @NotNull ConfigurationFile newConfig) {

        newConfig.set("yellow-number-in-tablist.enabled",
                !oldConfig.getString("yellow-number-in-tablist", TabConstants.Placeholder.PING).isEmpty());
        newConfig.set("yellow-number-in-tablist.value",
                oldConfig.getString("yellow-number-in-tablist", TabConstants.Placeholder.PING));
        newConfig.set("yellow-number-in-tablist.disable-in-worlds", oldConfig
                .getStringList("disable-features-in-worlds.yellow-number", Collections.singletonList("disabledworld")));
        if (TAB.getInstance().getPlatform().isProxy())
            newConfig.set("yellow-number-in-tablist.disable-in-servers", oldConfig.getStringList(
                    "disable-features-in-servers.yellow-number", Collections.singletonList("disabledserver")));
    ...
    private void convertBelowName(@NotNull ConfigurationFile oldConfig, @NotNull ConfigurationFile newConfig) {
        ...
        newConfig.set("belowname-objective.disable-in-worlds", oldConfig
                .getStringList("disable-features-in-worlds.belowname", Collections.singletonList("disabledworld")));
        if (TAB.getInstance().getPlatform().isProxy())
            newConfig.set("belowname-objective.disable-in-servers", oldConfig.getStringList(
                    "disable-features-in-servers.belowname", Collections.singletonList("disabledserver")));
        ...
        for (Map.Entry<String, List<Object>> entry : perWorldBossBars.entrySet()) {
            ...
            for (Object bar : entry.getValue()) {
                ...
                if (bars.get(bar).containsKey("display-condition")) {
                    ...
                    bars.get(bar).put("display-condition",
                            bars.get(bar).get("display-condition") + ";%" + separator + "%=" + entry.getKey());
    ...
    private void convertScoreboard(@NotNull ConfigurationFile newConfig, @NotNull ConfigurationFile premiumConfig) {
        ...
        for (Map.Entry<String, String> entry : perWorldScoreboards.entrySet()) {
            ...
            if (scoreboard.containsKey("display-condition")) {
                ...
                scoreboard.put("display-condition",
                        scoreboards.get(sb).get("display-condition") + ";%" + separator + "%=" + world);
    ...
    private void createDefaultScoreboard(@NotNull ConfigurationFile newConfig) {
        ...
        newConfig.set("scoreboard.scoreboards.admin.lines",
                Arrays.asList("%animation:MyAnimation1%", "&6Online:", "* &eOnline&7: &f%online%&7",
                        "* &eCurrent World&7: &f%worldonline%", "* &eStaff&7: &f%staffonline%", " ", "&6Server Info:",
                        "* &bTPS&7: %tps%", "* &bUptime&7: &f%server_uptime%",
                        "* &bMemory&7: &f%memory-used%&7/&4%memory-max%", "%animation:MyAnimation1%"));
        newConfig.set("scoreboard.scoreboards.scoreboard1.title", "Default");
        newConfig.set("scoreboard.scoreboards.scoreboard1.lines",
                Arrays.asList("%animation:MyAnimation1%", "&6My Stats:", "* &eKills&7: &f%statistic_player_kills%",
                        "* &eDeaths&7: &f%statistic_deaths%", "* &eHealth&7: &f%health%", " ", "&6Personal Info:",
                        "* &bRank&7: &f%group%", "* &bPing&7: &f%ping%&7ms", "* &bWorld&7: &f%world%",
    ...
    private void convertHeaderFooter(@NotNull ConfigurationFile oldConfig, @NotNull ConfigurationFile newConfig) {
        ...
        newConfig.set("header-footer.disable-in-worlds", oldConfig
                .getStringList("disable-features-in-worlds.header-footer", Collections.singletonList("disabledworld")));
        if (TAB.getInstance().getPlatform().isProxy())
            newConfig.set("header-footer.disable-in-servers", oldConfig.getStringList(
                    "disable-features-in-servers.header-footer", Collections.singletonList("disabledserver")));

        String separator = TAB.getInstance().getPlatform().isProxy() ? "server" : "world";
        Map<String, Map<String, Object>> perWorldSettings = oldConfig
                .getConfigurationSection("per-" + separator + "-settings");
        ...
        for (Map.Entry<String, Map<String, Object>> worldEntry : new LinkedHashMap<>(perWorldSettings).entrySet()) {
            ...
            for (Map.Entry<String, Object> propertyValueEntry : new LinkedHashMap<>(worldEntry.getValue()).entrySet()) {
                ...
                if (propertyValueEntry.getKey().equalsIgnoreCase("header")
                        || propertyValueEntry.getKey().equalsIgnoreCase("footer"))
    ...
    private void convertOtherOptions(@NotNull ConfigurationFile oldConfig, @NotNull ConfigurationFile newConfig,
            @Nullable ConfigurationFile premiumConfig)
    {
        ...
        if (premiumConfig != null) {
            ...
            newConfig.set("placeholder-output-replacements",
                    premiumConfig.getConfigurationSection("placeholder-output-replacements"));
        ...
        newConfig.set("placeholderapi-refresh-intervals",
                oldConfig.getConfigurationSection("placeholderapi-refresh-intervals"));
        newConfig.set("assign-groups-by-permissions", oldConfig.getBoolean("assign-groups-by-permissions", false));
        newConfig.set("primary-group-finding-list", oldConfig.getStringList("primary-group-finding-list",
                Arrays.asList("Owner", "Admin", "Mod", "Helper", "default")));
    ...
    private void convertGroupsAndUsers(@NotNull ConfigurationFile oldConfig, @NotNull ConfigurationFile groups,
            @NotNull ConfigurationFile users)
    {
    ...
    private String translateWorldGroup(@NotNull ConfigurationFile oldConfig, @NotNull String group) {
        ...
        String oldSeparator = oldConfig.hasConfigOption("multi-world-separator")
                ? oldConfig.getString("multi-world-separator")
                : "-";
    ...
    public void convert301to302(@NotNull ConfigurationFile config) {
        ...
        if (config.removeOption("placeholders.remove-strings")) {
            ...
            TAB.getInstance().getPlatform().logInfo(TabComponent.fromColoredText(
                    EnumChatFormat.YELLOW + "Performing configuration conversion from 3.0.1 to 3.0.2"));
    ...
    public void convert331to332(@NotNull ConfigurationFile config) {
        ...
        if (config.removeOption("scoreboard-teams.unlimited-nametag-mode.use-marker-tag-for-1-8-x-clients")) {
            ...
            TAB.getInstance().getPlatform().logInfo(TabComponent.fromColoredText(
                    EnumChatFormat.YELLOW + "Performing configuration conversion from 3.3.1 to 3.3.2)"));
    ...
    public void convert332to400(@NotNull ConfigurationFile config) throws IOException {
        ...
        if (config.hasConfigOption("fix-pet-names")) {
            ...
            TAB.getInstance().getPlatform().logInfo(TabComponent.fromColoredText(
                    EnumChatFormat.YELLOW + "Performing configuration conversion from 3.3.2 to 4.0.0"));
    ...
    public void convert403to404(@NotNull ConfigurationFile config) {
        ...
        if (config.removeOption("global-playerlist.update-latency")) {
            ...
            TAB.getInstance().getPlatform().logInfo(TabComponent.fromColoredText(
                    EnumChatFormat.YELLOW + "Performing configuration conversion from 4.0.3 to 4.0.4"));
    ...
    public void convert409to410(@NotNull ConfigurationFile config) {
        ...
        if (config.hasConfigOption("yellow-number-in-tablist")) {
            ...
            TAB.getInstance().getPlatform().logInfo(TabComponent.fromColoredText(
                    EnumChatFormat.YELLOW + "Performing configuration conversion from 4.0.9 to 4.1.0"));
