...
@Getter
public class LayoutManagerImpl extends TabFeature implements LayoutManager, JoinListener, QuitListener, VanishListener,
        Loadable, UnLoadable, Refreshable, TabListClearListener
{
    ...
    private final boolean remainingPlayersTextEnabled = config().getBoolean("layout.enable-remaining-players-text",
            true);
    private final String remainingPlayersText = EnumChatFormat
            .color(config().getString("layout.remaining-players-text", "... and %s more"));
    ...
    private final Map<TabPlayer, String> sortedPlayers = Collections
            .synchronizedMap(new TreeMap<>(Comparator.comparing(p -> p.layoutData.sortingString)));
    ...
    public void load() {
        ...
        TAB.getInstance().getFeatureManager().registerFeature(TabConstants.Feature.LAYOUT_LATENCY,
                new LayoutLatencyRefresher(this));
    ...
    private @NotNull Map<String, LayoutPattern> loadLayouts() {
        ...
        for (Entry<Object, Map<String, Object>> layout : config()
                .<Object, Map<String, Object>>getConfigurationSection("layout.layouts").entrySet())
        {
    ...
    public void onJoin(@NotNull TabPlayer p) {
        ...
        // Unformat original entries for players who can see a layout to avoid spaces
        // due to unparsed placeholders and
        // such
        if (highest == null)
            return;
    ...
    public void onTabListClear(@NotNull TabPlayer player) {
        ...
        if (player.layoutData.view != null)
            player.layoutData.view.send();
    ...
    public enum Direction {
        ...
        COLUMNS(slot -> slot), ROWS(slot -> (slot - 1) % 4 * 20 + (slot - ((slot - 1) % 4)) / 4 + 1);
