...
public class CpuReport {
    ...
    /**
     * Constructs new instance with given parameters and performs calculation and
     * ordering
     *
     * @param updateRateSeconds How often is a new report made
     * @param features          Feature usage map
     * @param placeholders      Placeholder usage map
     */
    public CpuReport(int updateRateSeconds, @NotNull Map<String, Map<String, AtomicLong>> features,
            @NotNull Map<String, AtomicLong> placeholders)
    {
        ...
        // No, it's not empty
        featureUsageTotal = featureUsage.values().stream()
                .mapToDouble(map -> map.values().stream().mapToDouble(Float::floatValue).sum()).sum();

        placeholderUsage = placeholders.entrySet().stream()
                .sorted(Map.Entry.comparingByValue((o1, o2) -> Long.compare(o2.get(), o1.get())))
                .collect(LinkedHashMap::new, (m, e) -> m.put(e.getKey(), (float) e.getValue().get() / TIME_PERCENT),
                        Map::putAll);
