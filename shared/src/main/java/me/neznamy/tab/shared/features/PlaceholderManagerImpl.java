...
public class PlaceholderManagerImpl extends TabFeature
    ...
    private final Map<String, Integer> refreshIntervals =
            config().getConfigurationSection("placeholderapi-refresh-intervals");
    ...
    private final TabExpansion tabExpansion =
            registerExpansion ? TAB.getInstance().getPlatform().createTabExpansion() : new EmptyTabExpansion();
    ...
    private void processRefreshResults(@NotNull PlaceholderRefreshTask task) {
        long time = System.nanoTime();
        Map<TabPlayer, Set<Refreshable>> update =
                new HashMap<>(TAB.getInstance().getOnlinePlayers().length + 1, 1);
        ...
        Map<TabPlayer, Set<Refreshable>> forceUpdate =
                updateRelationalPlaceholders(task.getRelationalPlaceholderResults());
    ...
    private void refreshFeatures(
            @NotNull Map<TabPlayer, Set<Refreshable>> forceUpdate, @NotNull Map<TabPlayer, Set<Refreshable>> update) {
    ...
    private Map<TabPlayer, Set<Refreshable>> updateRelationalPlaceholders(
        ...
        for (Entry<RelationalPlaceholderImpl, Map<TabPlayer, Map<TabPlayer, Object>>> entry : results.entrySet()) {
            ...
            for (Entry<TabPlayer, Map<TabPlayer, Object>> viewerResult :
                    entry.getValue().entrySet()) {
                TabPlayer viewer = viewerResult.getKey();
                if (!viewer.isOnline())
                    continue; // Player disconnected in the meantime while refreshing in another thread
                for (Entry<TabPlayer, Object> targetResult :
                        viewerResult.getValue().entrySet()) {
    ...
    private void updatePlayerPlaceholders(
            @NotNull Map<PlayerPlaceholderImpl, Map<TabPlayer, Object>> results,
    ...
    private void updateServerPlaceholders(
            @NotNull Map<ServerPlaceholderImpl, Object> results, @NotNull Map<TabPlayer, Set<Refreshable>> update) {
        if (results.isEmpty()) return;
    ...
    private void recalculateUsedPlaceholders() {
        usedPlaceholders = placeholderUsage.keySet().stream()
    ...
    public void onJoin(@NotNull TabPlayer connectedPlayer) {
        for (Placeholder p : usedPlaceholders) {
            if (p instanceof ServerPlaceholderImpl) { // server placeholders don't update on join
                tabExpansion.setPlaceholderValue(
                        connectedPlayer, p.getIdentifier(), ((ServerPlaceholderImpl) p).getLastValue());
            }
        }
        // Initialize to avoid onVanishStatusChange being called in the loop after joining because previous value was
        // null
    ...
    @Override
    public @NotNull ServerPlaceholderImpl registerServerPlaceholder(
    ...
    @Override
    public @NotNull PlayerPlaceholderImpl registerPlayerPlaceholder(
            @NonNull String identifier, int refresh, @NonNull Function<me.neznamy.tab.api.TabPlayer, Object> function) {
        ensureActive();
        ...
        if (p == null) {
            ...
            if (event.getServerPlaceholder() != null) {
                ...
                registerRelationalPlaceholder(
                        identifier, getRefreshInterval(identifier), event.getRelationalPlaceholder());
            } else {
