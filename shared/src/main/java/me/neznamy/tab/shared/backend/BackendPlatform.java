...
/**
 * Interface for backend platforms with a few default implementations, as well
 * as new methods.
 */
public interface BackendPlatform extends Platform {
     ...
     *
     * @param identifier Placeholder identifier to register
     */
    default void registerDummyPlaceholder(@NotNull String identifier) {
        ...
        if (identifier.startsWith("%rel_")) { // To prevent placeholder identifier check from throwing
            ...
            TAB.getInstance().getPlaceholderManager().registerRelationalPlaceholder(identifier, -1,
