...
public class GroupCommand extends PropertyCommand {
    ...
    private void sendGroupInfo(@Nullable TabPlayer sender, @NotNull String group) {
        ...
        for (Map.Entry<String, Object> entry : TAB.getInstance().getConfiguration().getGroups().getGlobalSettings(group)
                .entrySet())
        {
        ...
        for (Map.Entry<String, Map<String, Object>> entry : TAB.getInstance().getConfiguration().getGroups()
                .getPerWorldSettings(group).entrySet())
        {
        ...
        for (Map.Entry<String, Map<String, Object>> entry : TAB.getInstance().getConfiguration().getGroups()
                .getPerServerSettings(group).entrySet())
        {
    ...
    @Override
    public void saveEntity(@Nullable TabPlayer sender, @NotNull String group, @NotNull String type,
            @NotNull String value, @Nullable String server, @Nullable String world)
    {
        ...
        if (property.length > 0 && String.valueOf(value.isEmpty() ? null : value).equals(String.valueOf(property[0])))
            return;
        TAB.getInstance().getConfiguration().getGroups().setProperty(group, type, server, world,
    ...
    public @NotNull List<String> complete(@Nullable TabPlayer sender, @NotNull String[] arguments) {
        ...
        if (arguments.length == 1) {
            ...
            return groups.stream().filter(group -> group.toLowerCase().startsWith(arguments[0].toLowerCase()))
