...
public class BukkitTabCommand implements CommandExecutor, TabCompleter {
            ...
            @NotNull String[] args)
    {
        ...
        if (TAB.getInstance().isPluginDisabled()) {
            ...
            for (String message : TAB.getInstance().getDisabledCommand().execute(args,
                    sender.hasPermission(TabConstants.Permission.COMMAND_RELOAD),
                    sender.hasPermission(TabConstants.Permission.COMMAND_ALL)))
            {
            ...
            if (sender instanceof Player) {
                ...
                if (p == null)
                    return true; // player not loaded correctly
            ...
            @NotNull String[] args)
    {
        ...
        if (sender instanceof Player) {
            ...
            p = TAB.getInstance().getPlayer(((Player) sender).getUniqueId());
            if (p == null)
                return Collections.emptyList(); // player not loaded correctly
