...
public class BackendArmorStandManager implements ArmorStandManager {
     ...
     * @param owner    Owner of this armor stand manager
     */
    public BackendArmorStandManager(@NotNull NameTagX nameTagX, @NotNull TabPlayer owner) {
        ...
        owner.setProperty(nameTagX, TabConstants.Property.NAMETAG,
        ...
        for (Map.Entry<String, Object> line : nameTagX.getStaticLines().entrySet()) {
            ...
            armorStands.add(new ArmorStand((BackendNameTagX) nameTagX, this, owner, line.getKey(),
                    Double.parseDouble(line.getValue().toString()), true));
     ...
     *
     * @param viewer player to move armor stands for
     * @param diff   Move diff
     */
     ...
     *
     * @param viewer Player to check for
    ...
    public void respawn(@NotNull BackendTabPlayer viewer) {
        ...
        // creating new delayed task every time someone sneaks can be abused and cause
        // OOM
        // RIP 1.8.0
    ...
    /**
     * Spawns all armor stands for specified viewer and adds them into nearby
     * players
     *
    ...
    public void unregisterPlayer(@NotNull BackendTabPlayer viewer) {
        ...
        if (nearbyPlayerList.remove(viewer))
            nearbyPlayers = nearbyPlayerList.toArray(new BackendTabPlayer[0]);
    ...
    /**
     * Sends destroy packet of all armor stands to specified player and removes them
     * from nearby players list
     *
    ...
    public void updateMetadata(@NotNull BackendTabPlayer viewer) {
        ...
        for (ArmorStand a : armorStandArray) {
            ...
            viewer.getEntityView().updateEntityMetadata(a.entityId,
                    a.createDataWatcher(a.getProperty().getFormat(viewer), viewer));
