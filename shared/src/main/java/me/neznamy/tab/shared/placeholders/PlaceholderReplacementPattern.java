...
public class PlaceholderReplacementPattern {
    ...
    private static final PlaceholderReplacementPattern EMPTY = new PlaceholderReplacementPattern("",
            Collections.emptyMap());

    /**
     * Full replacement map with values colored and keys being duplicated, once with
     * and once without colors
     */
    private final Map<String, String> replacements = new HashMap<>();

    /**
     * Map of number intervals where key is a 2-dimensional array with first value
     * being minimum and second value maximum and value being output to replace to.
     */
     ...
     *
     * @param identifier placeholder identifier which this pattern belongs to
     * @param map        replacement map from config
     */
    private PlaceholderReplacementPattern(@NotNull String identifier, @NotNull Map<Object, Object> map) {
        ...
        for (Entry<Object, Object> entry : map.entrySet()) {
            ...
            // snakeyaml converts yes & no to booleans, making them not work when used
            // without "
            if ("true".equals(key)) {
                ...
                try {
                    ...
                    numberIntervals
                            .put(new float[]
                            { Float.parseFloat(key.split("-")[0]), Float.parseFloat(key.split("-")[1]) }, value);
    ...
    /**
     * Finds replacement using provided output as well as applying %value%
     * placeholder for original output inside replacements.
     *
     * @param output placeholder's output
    ...
    /**
     * Internal method that returns value based on provided placeholder output and
    ...
    private String findReplacement0(@NotNull String output) {
        ...
        // else
        if (replacements.containsKey("else"))
            return replacements.get("else");
