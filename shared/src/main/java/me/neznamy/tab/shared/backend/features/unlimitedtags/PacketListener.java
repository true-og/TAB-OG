 ...
 * Events are too unreliable and delayed/ahead which causes de-sync if trying to
 * listen to move event. For entering/leaving tracking range there are no events
 * and periodic / move-triggered distance checks would cause high CPU usage.
 */
...
public class PacketListener extends TabFeature implements JoinListener, QuitListener, Loadable {
     ...
     *
     * @param receiver Player who received the packet
    ...
    private void checkPacket(@NotNull BackendTabPlayer player, @NotNull Object packet) {
        ...
        if (player.getEntityView().isMovePacket(packet) && !player.getEntityView().isLookPacket(packet)) { // ignoring
                                                                                                           // head
                                                                                                           // rotation
                                                                                                           // only
                                                                                                           // packets

            onEntityMove(player, player.getEntityView().getMoveEntityId(packet),
                    player.getEntityView().getMoveDiff(packet));
    ...
    /**
     * Processes named entity spawn packet and spawns armor stands if entity ID
     * belongs to an online player.
     *
     * @param receiver packet receiver
     * @param entityId spawned entity
     */
    public void onEntitySpawn(@NotNull BackendTabPlayer receiver, int entityId) {
        ...
        if (spawnedPlayer != null && spawnedPlayer.isLoaded() && !nameTagX.isPlayerDisabled(spawnedPlayer)) {
            ...
            TAB.getInstance().getCPUManager().runMeasuredTask(getFeatureName(),
                    TabConstants.CpuUsageCategory.PACKET_ENTITY_SPAWN,
    ...
    /**
     * Processes entity move packet. If entity ID belongs to a player, armor stands
     * of that player are teleported to player who received the packet. If it
     * belongs to a vehicle carrying a player, that player's armor stands are
     * teleported as well.
     *
     * @param receiver     packet receiver
     * @param entityId     entity that moved
     * @param positionDiff Position difference
     */
    public void onEntityMove(@NotNull BackendTabPlayer receiver, int entityId, Location positionDiff) {
        ...
        if (pl != null) {
            ...
            // player moved
            if (nameTagX.isPlayerDisabled(pl) || !pl.isLoaded())
                return;
            BackendArmorStandManager asm = nameTagX.getArmorStandManager(pl);
            TAB.getInstance().getCPUManager().runMeasuredTask(getFeatureName(),
                    TabConstants.CpuUsageCategory.PACKET_PLAYER_MOVE, () -> asm.move(receiver, positionDiff));

        } else {

            // a non-player entity moved
            for (Integer entity : nameTagX.getVehicleManager().getVehicles().getOrDefault(entityId,
                    Collections.emptyList()))
            {
                ...
                if (passenger != null) {
                    ...
                    if (asm != null) {
                        ...
                        TAB.getInstance().getCPUManager().runMeasuredTask(getFeatureName(),
                                TabConstants.CpuUsageCategory.PACKET_ENTITY_MOVE_PASSENGER,
    ...
    /**
     * Processes entity teleport packet. If entity ID belongs to a player, armor
     * stands of that player are teleported to player who received the packet. If it
     * belongs to a vehicle carrying a player, that player's armor stands are
     * teleported as well.
     *
     * @param receiver packet receiver
    ...
    public void onEntityTeleport(@NotNull BackendTabPlayer receiver, int entityId) {
        ...
        if (pl != null) {
            ...
            BackendArmorStandManager asm = nameTagX.getArmorStandManager(pl);
            TAB.getInstance().getCPUManager().runMeasuredTask(getFeatureName(),
                    TabConstants.CpuUsageCategory.PACKET_PLAYER_MOVE, () -> asm.teleport(receiver));
    ...
    /**
     * Processes entity destroy packet and destroys armor stands if entity ID
     * belongs to an online player.
     *
     * @param receiver packet receiver
     * @param entities de-spawned entities
     */
    public void onEntityDestroy(@NotNull BackendTabPlayer receiver, int... entities) {
        ...
        for (int entity : entities) {
            ...
            if (deSpawnedPlayer != null && deSpawnedPlayer.isLoaded() && !nameTagX.isPlayerDisabled(deSpawnedPlayer)) {
                ...
                TAB.getInstance().getCPUManager().runMeasuredTask(getFeatureName(),
                        TabConstants.CpuUsageCategory.PACKET_ENTITY_DESTROY, () -> asm.destroy(receiver));
