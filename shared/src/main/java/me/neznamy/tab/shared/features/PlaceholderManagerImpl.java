 ...
 */
public class PlaceholderManagerImpl extends TabFeature
        implements PlaceholderManager, JoinListener, Loadable, Refreshable
{

    private final Pattern placeholderPattern = Pattern.compile("%([^%]*)%");

    private final boolean registerExpansion = config().getBoolean("placeholders.register-tab-expansion", true);
    private final Map<String, Integer> refreshIntervals = config()
    ...
    @NotNull
    @Getter
    private final TabExpansion tabExpansion = registerExpansion ? TAB.getInstance().getPlatform().createTabExpansion()
     ...
     *
     * @param cpu CPU manager for submitting tasks
     */
    ...
    private void refresh() {
        ...
        for (Placeholder placeholder : usedPlaceholders) {
            ...
            if (placeholder.getRefresh() == -1 || loopTime % placeholder.getRefresh() != 0)
                continue;
    ...
    private void processRefreshResults(@NotNull PlaceholderRefreshTask task) {
        ...
        Map<TabPlayer, Set<Refreshable>> forceUpdate = updateRelationalPlaceholders(
                task.getRelationalPlaceholderResults());
    ...
    private void refreshFeatures(@NotNull Map<TabPlayer, Set<Refreshable>> forceUpdate,
            ...
            @Nullable Map<RelationalPlaceholderImpl, Map<TabPlayer, Map<TabPlayer, Object>>> results)
    {
        ...
        for (Entry<RelationalPlaceholderImpl, Map<TabPlayer, Map<TabPlayer, Object>>> entry : results.entrySet()) {
            ...
            for (Entry<TabPlayer, Map<TabPlayer, Object>> viewerResult : entry.getValue().entrySet()) {
                ...
                if (!viewer.isOnline())
                    continue; // Player disconnected in the meantime while refreshing in another thread
                for (Entry<TabPlayer, Object> targetResult : viewerResult.getValue().entrySet()) {
                    ...
                    if (placeholder.hasValueChanged(viewer, target, targetResult.getValue())) {
                        ...
                        update.computeIfAbsent(target, x -> new HashSet<>())
                                .addAll(getPlaceholderUsage(placeholder.getIdentifier()));
        ...
        for (Entry<PlayerPlaceholderImpl, Map<TabPlayer, Object>> entry : results.entrySet()) {
            ...
            for (Entry<TabPlayer, Object> playerResult : entry.getValue().entrySet()) {
                ...
                if (placeholder.hasValueChanged(player, playerResult.getValue())) {
                    ...
                    update.computeIfAbsent(player, k -> new HashSet<>())
                            .addAll(getPlaceholderUsage(placeholder.getIdentifier()));
     ...
     * @param <T> Specific placeholder class
     */
    ...
    /**
     * Detects placeholders in text using %% pattern and returns list of all
     ...
     *
     * @param identifier Placeholder to mark as used
    ...
    private void recalculateUsedPlaceholders() {
        ...
        usedPlaceholders = placeholderUsage.keySet().stream().map(this::getPlaceholder).distinct()
     ...
     *
     * @param output      Output the placeholder has returned
     ...
     *
     * @param identifier Placeholder to get usage of
    ...
    public void onJoin(@NotNull TabPlayer connectedPlayer) {
        ...
        for (Placeholder p : usedPlaceholders) {
            ...
            if (p instanceof ServerPlaceholderImpl) { // server placeholders don't update on join
                ...
                tabExpansion.setPlaceholderValue(connectedPlayer, p.getIdentifier(),
                        ((ServerPlaceholderImpl) p).getLastValue());

            }

        }

        // Initialize to avoid onVanishStatusChange being called in the loop after
        // joining because previous value was
        // null
     ...
     *
     * @param identifier Placeholder to check
    ...
    @Override
    public @NotNull ServerPlaceholderImpl registerServerPlaceholder(@NonNull String identifier, int refresh,
    ...
    @Override
    public @NotNull RelationalPlaceholderImpl registerRelationalPlaceholder(@NonNull String identifier, int refresh,
    ...
    public @NotNull TabPlaceholder getPlaceholder(@NonNull String identifier) {
        ...
        if (p == null) {
            ...
            if (event.getServerPlaceholder() != null) {
                ...
                registerRelationalPlaceholder(identifier, getRefreshInterval(identifier),
                        event.getRelationalPlaceholder());
