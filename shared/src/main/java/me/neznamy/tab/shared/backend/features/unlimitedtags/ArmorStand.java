...
public class ArmorStand {
    ...
    /**
     * Visibility flag to compare against for previous value, including invisibility
     * potion compensation
     */
     ...
     *
     * @param asm          Armor stand manager which this armor stand belongs to
     * @param owner        Owner of the armor stand
     * @param propertyName Name of refresh property to use
     * @param yOffset      Offset in blocks
     * @param staticOffset {@code true} if offset is static, {@code false} if not
     */
    ...
    public void updateVisibility(boolean force) {
        ...
        boolean visibility = calculateVisibility() && !owner.hasInvisibilityPotion(); // trigger packet send but don't
                                                                                      // save, so it can be reverted if
                                                                                      // viewer
        // is spectator
    ...
    public boolean calculateVisibility() {
        ...
        if (manager.isArmorStandsAlwaysVisible())
            return true;
        if (manager.isOnBoat(owner))
            return false;
    ...
    /**
     * Returns {@code true} if display name is in fact empty, for example only
     * containing color codes, {@code false} if not.
     *
     ...
     *
     * @param sleeping Whether player is sleeping or not
     * @param sneaking Whether player is sneaking or not
     * @param viewer   Viewer of the armor stand
    ...
    public void updateMetadata() {
        ...
        for (BackendTabPlayer viewer : asm.getNearbyPlayers()) {
            ...
            viewer.getEntityView().updateEntityMetadata(entityId,
                    createDataWatcher(property.getFormat(viewer), viewer));
    ...
    public void sendTeleportPacket(@NotNull BackendTabPlayer viewer) {
        ...
        viewer.getEntityView().teleportEntity(entityId,
                new Location(manager.getX(owner), getYLocation(viewer), manager.getZ(owner)));
    ...
    /**
     * Returns Y location where armor stand should be at time of calling. This takes
     * into account everything that affects height, including viewer's game version.
     *
     * @param viewer Player looking at the armor stand
