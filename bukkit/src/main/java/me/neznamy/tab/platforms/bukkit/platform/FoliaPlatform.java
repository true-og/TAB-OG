                ...
                () ->
                {
                    ...
                    for (TabPlayer player : TAB.getInstance().getOnlinePlayers()) {
                        ...
                        if (!player.getWorld().equals(bukkitWorld)) {
                            ...
                            PerWorldPlayerList pwp = TAB.getInstance().getFeatureManager()
                                    .getFeature(TabConstants.Feature.PER_WORLD_PLAYER_LIST);
                            if (pwp != null) {

                                runSync((Entity) player.getPlayer(),
                                        () -> pwp.onWorldChange(new PlayerChangedWorldEvent((Player) player.getPlayer(),
    ...
    public void registerSyncPlaceholder(@NotNull String identifier, int refresh) {
        ...
        ppl[0] = TAB.getInstance().getPlaceholderManager().registerPlayerPlaceholder(identifier, refresh, p -> {
            ...
            runSync((Entity) p.getPlayer(), () -> {
                ...
                TAB.getInstance().getCPUManager().runTask(() -> ppl[0].updateValue(p, output)); // To ensure player is
                                                                                                // loaded
