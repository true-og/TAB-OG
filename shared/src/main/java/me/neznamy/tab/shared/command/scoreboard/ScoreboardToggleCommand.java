...
public class ScoreboardToggleCommand extends SubCommand {
    ...
    public void execute(@Nullable TabPlayer sender, @NotNull String[] args) {
        ...
        ScoreboardManagerImpl scoreboard = TAB.getInstance().getFeatureManager()
        ...
        if (target.scoreboardData.otherPluginScoreboard != null)
            return; // not overriding other plugins
    ...
    public @NotNull List<String> complete(@Nullable TabPlayer sender, @NotNull String[] arguments) {
        ...
        if (arguments.length == 2)
            return getStartingArgument(Collections.singletonList("-s"), arguments[1]);
