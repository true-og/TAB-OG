...
/**
 * Feature handler for scoreboard objective with PLAYER_LIST display slot (in
 * tablist).
 */
        ...
        implements JoinListener, Loadable, UnLoadable, Refreshable, LoginPacketListener
{
    ...
    private static final String TITLE = "PlayerListObjectiveTitle"; // Unused by this objective slot (on Java, only
                                                                    // visible on Bedrock)
    ...
    /**
     * Constructs new instance and registers disable condition checker to feature
     * manager.
     */
    public YellowNumber() {

        Condition disableCondition = Condition
                .getCondition(config().getString("playerlist-objective.disable-condition"));
        disableChecker = new DisableChecker(getFeatureName(), disableCondition, this::onDisableConditionChange,
                p -> p.disabledYellowNumber);
        TAB.getInstance().getFeatureManager().registerFeature(TabConstants.Feature.YELLOW_NUMBER + "-Condition",
                disableChecker);
    ...
    public void load() {
        ...
        for (TabPlayer viewer : TAB.getInstance().getOnlinePlayers()) {
            ...
            for (Map.Entry<TabPlayer, Integer> entry : values.entrySet()) {
                ...
                setScore(viewer, entry.getKey(), entry.getValue(),
    ...
    public void onJoin(@NotNull TabPlayer connectedPlayer) {
        ...
        for (TabPlayer all : TAB.getInstance().getOnlinePlayers()) {
            ...
            if (all != connectedPlayer) {
                ...
                setScore(connectedPlayer, all, getValueNumber(all),
        ...
        if (redis != null)
            redis.updateYellowNumber(connectedPlayer, value, valueFancy.get());
    ...
    public void refresh(@NotNull TabPlayer refreshed, boolean force) {
        ...
        if (redis != null)
            redis.updateYellowNumber(refreshed, value, fancy.get());
    ...
    public void onLoginPacket(@NotNull TabPlayer p) {
        ...
        if (p.disabledYellowNumber.get() || !p.isLoaded())
            return;
        register(p);
        for (TabPlayer all : TAB.getInstance().getOnlinePlayers()) {

            if (all.isLoaded())
                setScore(p, all, getValueNumber(all), all.getProperty(PROPERTY_VALUE_FANCY).getFormat(p));
            ...
            @NotNull String fancyValue)
    {
        ...
        if (viewer.disabledYellowNumber.get())
            return;
