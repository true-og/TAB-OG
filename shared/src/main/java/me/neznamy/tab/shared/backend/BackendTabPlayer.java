...
/**
 * TabPlayer extension for backend platforms, which have access to more data and
 * can display it.
 */
public abstract class BackendTabPlayer extends TabPlayer {
            ...
            @NotNull String name, @NotNull String world, int serverVersion)
    {
        ...
        super(platform, player, uniqueId, name, TAB.getInstance().getConfiguration().getServerName(), world,
                ViaVersionHook.getInstance().getPlayerVersion(uniqueId, name, serverVersion), true);

    }

    /**
     * Returns player's health for
     * {@link me.neznamy.tab.shared.TabConstants.Placeholder#HEALTH} placeholder.
     *
    ...
    /**
     * Returns player's display name for
     * {@link me.neznamy.tab.shared.TabConstants.Placeholder#DISPLAY_NAME}
     * placeholder.
     *
