...
public class MySQLUserConfiguration implements PropertyConfiguration {
    ...
    private void setProperty0(
            @NotNull TabPlayer user,
            @NotNull String property,
            @Nullable String server,
            @Nullable String world,
            @Nullable String value) {
        if (world != null) {
            perWorld.computeIfAbsent(world, w -> new WeakHashMap<>())
                    .computeIfAbsent(user, g -> new HashMap<>())
                    .put(property, value);
        } else if (server != null) {
            perServer
                    .computeIfAbsent(server, s -> new WeakHashMap<>())
                    .computeIfAbsent(user, g -> new HashMap<>())
                    .put(property, value);
        } else {
    ...
    public String[] getProperty(
            @NotNull String user, @NotNull String property, @Nullable String server, @Nullable String world) {
        TabPlayer p = getPlayer(user);
        Object value;
        if ((value = perWorld.getOrDefault(world, new WeakHashMap<>())
        ...
        if ((value = perServer
                        .getOrDefault(server, new WeakHashMap<>())
                        .getOrDefault(p, new HashMap<>())
                        .get(property))
                != null) {
    ...
    public void load(@NotNull TabPlayer player) {
        TAB.getInstance().getCPUManager().runTask(() -> {
            try {
                CachedRowSet crs = mysql.getCRS(
                        "select * from `tab_users` where `user` = ?",
