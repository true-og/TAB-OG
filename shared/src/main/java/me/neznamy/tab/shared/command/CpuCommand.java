...
public class CpuCommand extends SubCommand {
    ...
    public void execute(@Nullable TabPlayer sender, @NotNull String[] args) {
        ...
        sendMessage(sender, "&8&l" + LINE_CHAR + "&8&m                                                    ");
        sendMessage(sender, String.format("&8&l%s &6&lPlaceholders Total: &a&l%s%%", LINE_CHAR,
                colorize(decimal3.format(report.getPlaceholderUsageTotal()), 10, 5)));
        sendMessage(sender, String.format("&8&l%s &6&lPlugin internals: &a&l%s%%", LINE_CHAR,
                colorize(decimal3.format(report.getFeatureUsageTotal() - report.getPlaceholderUsageTotal()), 10, 5)));
        sendMessage(sender, String.format("&8&l%s &6&lTotal: &e&l%s%%", LINE_CHAR,
                colorize(decimal3.format(report.getFeatureUsageTotal()), 10, 5)));
        sendMessage(sender, "&8&l" + LINE_CHAR + "&8&m             &r&8&l[ &bTAB CPU Stats &8&l]&r&8&l&m             ");
        sendMessage(sender, " ");
    ...
    private void printPlaceholders(@Nullable TabPlayer sender, @NotNull Map<String, Float> map) {
        ...
        for (Entry<String, Float> entry : map.entrySet()) {
            ...
            String colorized = entry.getKey().startsWith("%sync:") ? "&c" + decimal3.format(entry.getValue())
        ...
        for (Entry<String, Map<String, Float>> entry : features.entrySet()) {
            ...
            TAB.getInstance().getPlatform()
                    .logInfo(TabComponent.fromColoredText(
                            EnumChatFormat.color(String.format("&8&l%s &7%s &7(%s%%&7):", LINE_CHAR, entry.getKey(),
                                    colorize(decimal3.format(
                                            entry.getValue().values().stream().mapToDouble(Float::floatValue).sum()), 5,
                                            1)))));
            for (Entry<String, Float> type : entry.getValue().entrySet()) {

                TAB.getInstance().getPlatform().logInfo(
                        TabComponent.fromColoredText(EnumChatFormat.color(String.format("&8&l%s     &7%s - %s%%",
    ...
    public void sendToPlayer(@NotNull TabPlayer sender, @NotNull Map<String, Map<String, Float>> features) {
        ...
        for (Entry<String, Map<String, Float>> entry : features.entrySet()) {
            ...
            String core = String.format("&8&l%s &7%s &7(%s%%&7):", LINE_CHAR, entry.getKey(),
                    colorize(decimal3.format(featureTotal), 5, 1));
