...
public class CpuReport {
    ...
    public CpuReport(
            ...
            @NotNull Map<String, Map<String, AtomicLong>> features,
        ...
        featureUsage = sorted.values().stream()
                .collect(
                        () -> new LinkedHashMap<>(features.size()),
        ...
        featureUsageTotal = featureUsage.values().stream()
                .mapToDouble(map ->
                        map.values().stream().mapToDouble(Float::floatValue).sum())
                .sum();
        ...
        placeholderUsage = placeholders.entrySet().stream()
                ...
                .collect(
                        LinkedHashMap::new,
                        (m, e) -> m.put(e.getKey(), (float) e.getValue().get() / TIME_PERCENT),
                        Map::putAll);

        placeholderUsageTotal = placeholderUsage.values().stream()
