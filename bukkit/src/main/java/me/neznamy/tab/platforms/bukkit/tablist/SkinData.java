...
public class SkinData {
    ...
    // #2 - Random mods may perform deep hack into the server and add another one
    // (see #1089)
    // Get first and hope for the best, alternatively players may not have correct
    // skins in layout, but who cares
    private final Method getProfile = ReflectionUtils.getMethods(
            BukkitReflection.getClass("world.entity.player.Player", "world.entity.player.EntityHuman", "EntityHuman"),
            GameProfile.class).get(0);

    /**
     * Constructs new instance and loads NMS fields. If it fails, throws an
     * Exception.
     *
     ...
     *
     * @param player Player to get skin of
    ...
    public TabList.Skin getSkin(@NotNull Player player) {
        ...
        Collection<Property> col = ((GameProfile) getProfile.invoke(getHandle.invoke(player))).getProperties()
                .get(TabList.TEXTURES_PROPERTY);
        if (col.isEmpty())
            return null; // offline mode
        Property property = col.iterator().next();
        if (BukkitReflection.is1_20_2Plus()) {

            return new TabList.Skin((String) property.getClass().getMethod("value").invoke(property),
