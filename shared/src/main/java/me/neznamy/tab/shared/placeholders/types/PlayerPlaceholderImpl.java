    ...
    public Object request(@NonNull TabPlayer p) {
        ...
        try {
            ...
            TAB.getInstance()
                    .getErrorManager()
                    .placeholderError(
                            "Player placeholder " + identifier + " generated an error when setting for player "
                                    + p.getName(),
                            t);
            return ERROR_VALUE;
        } finally {
            ...
            if (timeDiff > TabConstants.Placeholder.RETURN_TIME_WARN_THRESHOLD) {
                TAB.getInstance()
                        .debug("Placeholder " + identifier + " took " + timeDiff + "ms to return value for player "
                                + p.getName());
            }
        }
    }
