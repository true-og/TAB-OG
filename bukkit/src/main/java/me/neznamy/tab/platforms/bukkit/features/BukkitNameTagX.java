...
public class BukkitNameTagX extends BackendNameTagX implements Listener {
     ...
     *
     * @param plugin Plugin instance to use for registering listener
     */
    ...
    public boolean isSwimming(@NotNull TabPlayer player) {
        ...
        if (BukkitReflection.getMinorVersion() >= 14 && p.getPose() == Pose.SWIMMING)
            return true;
    ...
    @Override
    @NotNull
    public EntityData createDataWatcher(@NotNull TabPlayer viewer, byte flags, @NotNull String displayName,
            boolean nameVisible)
    {
