...
/**
 * Line using all 3 values - prefix, name and suffix. Line may flicker when
 * placeholder changes value. Limitations: 1.5.x - 1.7.x: up to 42 characters
 * 1.8.x - 1.12.x: up to 66 characters 1.13+: unlimited
 */
public class LongLine extends ScoreboardLine {
    ...
    public void refresh(@NotNull TabPlayer refreshed, boolean force) {
        ...
        if (refreshed.getProperty(textProperty).update()) {
            ...
            if (refreshed.getVersion().getMinorVersion() >= 13) {
                ...
                String[] values = splitText(getPlayerName(lineNumber),
                        RGBUtils.getInstance().convertRGBtoLegacy(refreshed.getProperty(textProperty).get()),
                        refreshed.getVersion().getMinorVersion() >= 8 ? Limitations.SCOREBOARD_SCORE_LENGTH_1_8
    ...
    public void register(@NonNull TabPlayer p) {
        ...
        if (p.getVersion().getMinorVersion() >= 13) {
            ...
            String[] values = splitText(playerName, RGBUtils.getInstance().convertRGBtoLegacy(value),
                    p.getVersion().getMinorVersion() >= 8 ? Limitations.SCOREBOARD_SCORE_LENGTH_1_8
                            : Limitations.SCOREBOARD_SCORE_LENGTH_1_7);
