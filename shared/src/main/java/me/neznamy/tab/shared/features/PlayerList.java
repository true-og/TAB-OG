 ...
 */
@Getter
public class PlayerList extends TabFeature implements TabListFormatManager, JoinListener, Loadable, UnLoadable,
        WorldSwitchListener, ServerSwitchListener, Refreshable, VanishListener
{

    /**
     * Config option toggling anti-override which prevents other plugins from
     * overriding TAB
     */
    protected final boolean antiOverrideTabList = config().getBoolean("tablist-name-formatting.anti-override", true);

    private final LayoutManagerImpl layoutManager = TAB.getInstance().getFeatureManager()
            .getFeature(TabConstants.Feature.LAYOUT);
    private RedisSupport redis;
    protected final DisableChecker disableChecker;

    /**
     * Flag tracking when the plugin is disabling to properly clear display name by
     * setting it to null value and not force the value back with the anti-override.
     */
    private boolean disabling;

    /**
     * Constructs new instance, registers disable checker into feature manager and
     * starts anti-override.
     */
    public PlayerList() {

        Condition disableCondition = Condition
                .getCondition(config().getString("tablist-name-formatting.disable-condition"));
        disableChecker = new DisableChecker(getFeatureName(), disableCondition, this::onDisableConditionChange,
                p -> p.disabledPlayerList);
        TAB.getInstance().getFeatureManager().registerFeature(TabConstants.Feature.PLAYER_LIST + "-Condition",
                disableChecker);
    ...
    /**
     * Returns UUID of tablist entry representing this player. If layout feature is
     * enabled, returns UUID of the layout slot where the player should be. When
     * it's not enabled, returns player's TabList UUID, which may not match with
     * player's actual UUID due to velocity.
     *
     * @param p      Player to get tablist UUID of
    ...
    /**
     * Loads all properties from config and returns {@code true} if at least one of
     * them either wasn't loaded or changed value, {@code false} otherwise.
     *
     * @param p Player to update properties of
    ...
    protected boolean updateProperties(@NotNull TabPlayer p) {
        ...
        if (p.loadPropertyFromConfig(this, TabConstants.Property.CUSTOMTABNAME, p.getName()))
            changed = true;
        if (p.loadPropertyFromConfig(this, TabConstants.Property.TABSUFFIX))
            changed = true;
        return changed;
     ...
     * @param format Whether player's actual format should be used or {@code null}
     *               for reset
     */
    protected void updatePlayer(@NotNull me.neznamy.tab.api.TabPlayer p, boolean format) {
        ...
        if (redis != null)
            redis.updateTabFormat(player,
                    player.getProperty(TabConstants.Property.TABPREFIX).get()
     ...
     *
     * @param p      Player to get format of
     * @param viewer Viewer seeing the format
    ...
    public void load() {
        ...
        for (TabPlayer all : TAB.getInstance().getOnlinePlayers()) {
            ...
            if (disableChecker.isDisableConditionMet(all)) {
                ...
                if (redis != null)
                    redis.updateTabFormat(all,
                            all.getProperty(TabConstants.Property.TABPREFIX).get()
                                    + all.getProperty(TabConstants.Property.CUSTOMTABNAME).get()
                                    + all.getProperty(TabConstants.Property.TABSUFFIX).get());
        ...
        for (TabPlayer viewer : TAB.getInstance().getOnlinePlayers()) {
            ...
            for (TabPlayer target : TAB.getInstance().getOnlinePlayers()) {
                ...
                if (target.disabledPlayerList.get())
                    continue;
    ...
    public void onServerChange(@NotNull TabPlayer p, @NotNull String from, @NotNull String to) {
        ...
        if (updateProperties(p) && !p.disabledPlayerList.get())
            updatePlayer(p, true);
    ...
    public void onWorldChange(@NotNull TabPlayer changed, @NotNull String from, @NotNull String to) {
        ...
        if (updateProperties(changed) && !changed.disabledPlayerList.get())
            updatePlayer(changed, true);
    ...
    public void refresh(@NotNull TabPlayer refreshed, boolean force) {
        ...
        if (refreshed.getProperty(TabConstants.Property.TABPREFIX) == null)
            return; // Placeholder in condition on join
        boolean refresh;
        if (force) {
        ...
        if (refreshed.disabledPlayerList.get())
            return;
        if (refresh) {
    ...
    public void onJoin(@NotNull TabPlayer connectedPlayer) {
        ...
        Runnable r = () -> {
            ...
            for (TabPlayer all : TAB.getInstance().getOnlinePlayers()) {
                ...
                connectedPlayer.getTabList().updateDisplayName(getTablistUUID(all, connectedPlayer),
                        getTabFormat(all, connectedPlayer));

            }

        };
        // add packet might be sent after tab's refresh packet, resending again when
        // anti-override is disabled
        if (!antiOverrideTabList
                || !TAB.getInstance().getFeatureManager().isFeatureEnabled(TabConstants.Feature.PIPELINE_INJECTION))
        {

            TAB.getInstance().getCPUManager().runTaskLater(300, getFeatureName(),
                    TabConstants.CpuUsageCategory.PLAYER_JOIN, r);
    ...
    public void onVanishStatusChange(@NotNull TabPlayer player) {
        ...
        if (player.isVanished() || player.disabledPlayerList.get())
            return;
        for (TabPlayer viewer : TAB.getInstance().getOnlinePlayers()) {
