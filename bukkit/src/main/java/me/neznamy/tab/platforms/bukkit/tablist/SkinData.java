...
public class SkinData {
    ...
    private final Method getProfile = ReflectionUtils.getMethods(
                    BukkitReflection.getClass(
                            "world.entity.player.Player", "world.entity.player.EntityHuman", "EntityHuman"),
                    GameProfile.class)
            .get(0);
    ...
    public TabList.Skin getSkin(@NotNull Player player) {
        Collection<Property> col = ((GameProfile) getProfile.invoke(getHandle.invoke(player)))
                .getProperties()
