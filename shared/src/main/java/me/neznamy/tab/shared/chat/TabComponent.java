...
public abstract class TabComponent {
    ...
    /**
     * Component cache maps to avoid large memory allocations as well as higher CPU
     * usage when using animations which send the same text on repeat.
     */
    private static final ComponentCache<String, TabComponent> stringCache = new ComponentCache<>(1000,
            (text, clientVersion) ->
            {

                return text.contains("#") || text.contains("&x") || text.contains(EnumChatFormat.COLOR_CHAR + "x")
                        || text.contains("<") ? fromColoredText(text) : // contains RGB colors or font
     ...
     * @param <T> Platform's component class
     */
    @NotNull
    ...
    /**
     * Converts this component into a simple text with legacy colors (the closest
     * match if color is set to RGB)
     *
    ...
    /**
     * Converts this component into a string that only consists of text without any
     * formatting.
     *
    ...
    /**
     * Returns the most optimized component based on text. Returns null if text is
     * null, organized component if RGB colors are used or simple component with
     * only text field containing the whole text when no RGB colors are used
     *
    ...
    public static TabComponent fromColoredText(@NotNull String originalText) {
        ...
        while (!remainingText.isEmpty()) {
            ...
            if (m.find()) {
                ...
                components.addAll(toComponentArray(match.substring(match.indexOf('>') + 1, match.length() - 7),
    ...
    /**
     * Returns true if entered string is a valid 6-digit combination of hexadecimal
     * numbers, false if not
     *
