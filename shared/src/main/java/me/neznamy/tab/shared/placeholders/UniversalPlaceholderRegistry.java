...
/**
 * An implementation of PlaceholderRegistry for universal placeholders which
...
public class UniversalPlaceholderRegistry {
    ...
    private void registerConstants(@NotNull PlaceholderManager manager) {
        ...
        manager.registerPlayerPlaceholder(TabConstants.Placeholder.PLAYER_VERSION, -1,
                p -> ((TabPlayer) p).getVersion().getFriendlyName());
        manager.registerPlayerPlaceholder(TabConstants.Placeholder.PLAYER_VERSION_ID, -1,
                p -> ((TabPlayer) p).getVersion().getNetworkId());

        // Server
        manager.registerServerPlaceholder("%%", -1, () -> "%");
        manager.registerServerPlaceholder(TabConstants.Placeholder.MEMORY_MAX, -1,
                () -> ((int) (Runtime.getRuntime().maxMemory() / 1024 / 1024)));
        manager.registerServerPlaceholder(TabConstants.Placeholder.MEMORY_MAX_GB, -1,
                () -> decimal2.format((float) Runtime.getRuntime().maxMemory() / 1024 / 1024 / 1024));
    ...
    private void registerServerPlaceholders(@NotNull PlaceholderManager manager) {
        ...
        SimpleDateFormat timeFormat = createDateFormat(TAB.getInstance().getConfiguration().getConfig()
                .getString("placeholders.time-format", "[HH:mm:ss / h:mm a]"), "[HH:mm:ss / h:mm a]");
        SimpleDateFormat dateFormat = createDateFormat(
                TAB.getInstance().getConfiguration().getConfig().getString("placeholders.date-format", "dd.MM.yyyy"),
                "dd.MM.yyyy");
        manager.registerServerPlaceholder(TabConstants.Placeholder.TIME, 500,
                () -> timeFormat.format(new Date(System.currentTimeMillis() + (int) (timeOffset * 3600000))));
        manager.registerServerPlaceholder(TabConstants.Placeholder.DATE, 60000,
                () -> dateFormat.format(new Date(System.currentTimeMillis() + (int) (timeOffset * 3600000))));
        manager.registerServerPlaceholder(TabConstants.Placeholder.MEMORY_USED, 200,
                () -> ((int) ((Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory()) / 1024 / 1024)));
        manager.registerServerPlaceholder(TabConstants.Placeholder.MEMORY_USED_GB, 200, () -> decimal2.format(
                (float) (Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory()) / 1024 / 1024 / 1024));
        ...
        manager.registerServerPlaceholder(TabConstants.Placeholder.STAFF_ONLINE, 2000, () -> {
            ...
            for (TabPlayer player : TAB.getInstance().getOnlinePlayers()) {
                ...
                if (player.hasPermission(TabConstants.Permission.STAFF) && !player.isVanished())
                    count++;
    ...
    private void registerPlayerPlaceholders(@NotNull PlaceholderManager manager) {
        ...
        manager.registerPlayerPlaceholder(TabConstants.Placeholder.WORLD_ONLINE, 1000, p -> {
            ...
            for (TabPlayer player : TAB.getInstance().getOnlinePlayers()) {
                ...
                if (((TabPlayer) p).getWorld().equals(player.getWorld()) && !player.isVanished())
                    count++;
        ...
        manager.registerPlayerPlaceholder(TabConstants.Placeholder.SERVER_ONLINE, 1000, p -> {
            ...
            for (TabPlayer player : TAB.getInstance().getOnlinePlayers()) {
                ...
                if (((TabPlayer) p).getServer().equals(player.getServer()) && !player.isVanished())
                    count++;
        ...
        for (Object s : TAB.getInstance().getConfiguration().getAnimationFile().getValues().keySet()) {
            ...
            Animation a = new Animation((PlaceholderManagerImpl) manager, s.toString(),
            ...
            manager.registerPlayerPlaceholder(TabConstants.Placeholder.animation(a.getName()), a.getRefresh(),
                    p -> a.getMessage());

        }

        Condition.clearConditions();
        Map<String, Map<Object, Object>> conditions = TAB.getInstance().getConfiguration().getConfig()
                .getConfigurationSection("conditions");
        for (Entry<String, Map<Object, Object>> condition : conditions.entrySet()) {
            ...
            manager.registerPlayerPlaceholder(TabConstants.Placeholder.condition(c.getName()), c.getRefresh(),
    ...
    /**
     * Evaluates inserted date format. If it's not valid, a message is printed into
     * console and format with {@code defaultValue} is returned.
     *
     * @param value        date format to evaluate
     * @param defaultValue value to use if entered format is not valid
