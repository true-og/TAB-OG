...
public class NameTag extends TabFeature implements NameTagManager, JoinListener, QuitListener, Loadable, UnLoadable,
        WorldSwitchListener, ServerSwitchListener, Refreshable, LoginPacketListener, VanishListener
{

    protected final boolean invisibleNameTags = config().getBoolean("scoreboard-teams.invisible-nametags", false);
    private final boolean canSeeFriendlyInvisibles = config().getBoolean("scoreboard-teams.can-see-friendly-invisibles",
            false);
    ...
    public NameTag() {
        ...
        disableChecker = new DisableChecker(getFeatureName(), disableCondition, this::onDisableConditionChange,
                p -> p.disabledNametags);
        TAB.getInstance().getFeatureManager().registerFeature(TabConstants.Feature.NAME_TAGS + "-Condition",
                disableChecker);
        if (!antiOverride)
    ...
    public void load() {
        ...
        TAB.getInstance().getFeatureManager().registerFeature(TabConstants.Feature.NAME_TAGS_COLLISION,
                collisionManager);
        ...
        TAB.getInstance().getFeatureManager().registerFeature(TabConstants.Feature.NAME_TAGS_VISIBILITY,
                new VisibilityRefresher(this));
        ...
        for (TabPlayer viewer : TAB.getInstance().getOnlinePlayers()) {
            ...
            for (TabPlayer target : TAB.getInstance().getOnlinePlayers()) {
                ...
                if (!target.disabledNametags.get())
                    registerTeam(target, viewer);
    ...
    public void unload() {
        ...
        for (TabPlayer viewer : TAB.getInstance().getOnlinePlayers()) {
            ...
            for (TabPlayer target : TAB.getInstance().getOnlinePlayers()) {
                ...
                if (hasTeamHandlingPaused(target) || target.disabledNametags.get())
                    continue;
    ...
    public void onJoin(@NotNull TabPlayer connectedPlayer) {
        ...
        for (TabPlayer all : TAB.getInstance().getOnlinePlayers()) {
            ...
            if (all == connectedPlayer)
                continue; // avoiding double registration
    ...
    public void onQuit(@NotNull TabPlayer disconnectedPlayer) {
        ...
        if (!disconnectedPlayer.disabledNametags.get() && !hasTeamHandlingPaused(disconnectedPlayer)) {
            ...
            for (TabPlayer viewer : TAB.getInstance().getOnlinePlayers()) {
                ...
                if (viewer == disconnectedPlayer)
                    continue; // player who just disconnected
    ...
    public void onServerChange(@NonNull TabPlayer p, @NonNull String from, @NonNull String to) {
        ...
        if (updateProperties(p) && !p.disabledNametags.get())
            updateTeamData(p);
    ...
    public void onWorldChange(@NotNull TabPlayer changed, @NotNull String from, @NotNull String to) {
        ...
        if (updateProperties(changed) && !changed.disabledNametags.get())
            updateTeamData(changed);
    ...
    public void updateTeamData(@NonNull TabPlayer p) {
        ...
        if (redis != null)
            redis.updateTeam(p, p.sortingData.getShortTeamName(), p.getProperty(TabConstants.Property.TAGPREFIX).get(),
    ...
    public void updateTeamData(@NonNull TabPlayer p, @NonNull TabPlayer viewer) {
        ...
        viewer.getScoreboard().updateTeam(p.sortingData.getShortTeamName(), prefix,
                p.getProperty(TabConstants.Property.TAGSUFFIX).getFormat(viewer),
                visible ? NameVisibility.ALWAYS : NameVisibility.NEVER,
                p.teamData.getCollisionRule() ? CollisionRule.ALWAYS : CollisionRule.NEVER, teamOptions,
    ...
    private void registerTeam(@NonNull TabPlayer p, @NonNull TabPlayer viewer) {
        ...
        if (!TAB.getInstance().getPlatform().canSee(viewer, p) && p != viewer)
            return;
        String prefix = p.getProperty(TabConstants.Property.TAGPREFIX).getFormat(viewer);
        viewer.getScoreboard().registerTeam(p.sortingData.getShortTeamName(), prefix,
                p.getProperty(TabConstants.Property.TAGSUFFIX).getFormat(viewer),
                ...
                Collections.singletonList(p.getNickname()), teamOptions, EnumChatFormat.lastColorsOf(prefix));
    ...
    protected boolean updateProperties(@NonNull TabPlayer p) {
        ...
        if (p.loadPropertyFromConfig(this, TabConstants.Property.TAGSUFFIX))
            changed = true;
        return changed;
    ...
    public void onLoginPacket(TabPlayer player) {
        ...
        for (TabPlayer all : TAB.getInstance().getOnlinePlayers()) {
            ...
            if (!all.disabledNametags.get() && all.isLoaded())
                registerTeam(all, player);
    ...
    public void toggleNameTagVisibilityView(@NonNull me.neznamy.tab.api.TabPlayer p, boolean sendToggleMessage) {
        ...
        TAB.getInstance().getPlaceholderManager().getTabExpansion().setNameTagVisibility(player,
                !player.teamData.invisibleNameTagView);
        for (TabPlayer all : TAB.getInstance().getOnlinePlayers()) {
    ...
    public static class PlayerData {
        ...
        /**
         * Flag tracking whether this player has name tag hidden globally using API or
         * not
         */
        ...
        /**
         * Returns current collision rule. If forced using API, the forced value is
         * returned. Otherwise, value assigned internally based on configuration is
         * returned.
         *
