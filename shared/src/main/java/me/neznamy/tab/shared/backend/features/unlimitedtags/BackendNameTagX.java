...
public abstract class BackendNameTagX extends NameTagX implements GameModeListener, PacketSendListener {
    ...
    protected BackendNameTagX() {
        ...
        TAB.getInstance().getFeatureManager().registerFeature(TabConstants.Feature.UNLIMITED_NAME_TAGS_PACKET_LISTENER,
                packetListener);

    }

    /**
     * Starts task checking for player visibility to hide armor stands of invisible
     * players.
     */
    private void startVisibilityRefreshTask() {

        TAB.getInstance().getCPUManager().startRepeatingMeasuredTask(500, getExtraFeatureName(),
    ...
    public void onJoin(@NotNull TabPlayer connectedPlayer) {
        ...
        if (isPlayerDisabled(connectedPlayer))
            return;
        for (TabPlayer viewer : TAB.getInstance().getOnlinePlayers()) {
     ...
     * These include players being in the same world, distance being less than 48
     * blocks and target player being visible to viewer.
     *
     * @param viewer Player viewing armor stands
     * @param target Target player with armor stands
     */
    private void spawnArmorStands(@NotNull TabPlayer viewer, @NotNull TabPlayer target) {

        if (viewer.getVersion().getMinorVersion() < 8)
            return;
        if (target == viewer || isPlayerDisabled(target) || isDead(target))
            return;
        if (!areInSameWorld(viewer, target))
            return;
    ...
    public void sneak(UUID playerUUID, boolean sneaking) {
        ...
        if (p == null || isPlayerDisabled(p))
            return;
        TAB.getInstance().getCPUManager().runMeasuredTask(getExtraFeatureName(),
                TabConstants.CpuUsageCategory.PLAYER_SNEAK, () ->
                {
    ...
    public void respawn(UUID playerUUID) {
        ...
        if (respawned == null || isPlayerDisabled(respawned))
            return;
        TAB.getInstance().getCPUManager().runMeasuredTask(getExtraFeatureName(),
                TabConstants.CpuUsageCategory.PLAYER_RESPAWN, () -> getArmorStandManager(respawned).teleport());
    ...
    public abstract EntityData createDataWatcher(@NotNull TabPlayer viewer, byte flags, @NotNull String displayName,
            boolean nameVisible);
