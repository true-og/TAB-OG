...
public abstract class SortingType {
    ...
    /**
     * Number to add to / subtract from to prevent incorrect sorting with negative
     * values
     */
     ...
     *
     * @param elements Configured sorting values
     ...
     * a number in a base of 65534. The first two represent the whole part, the
     * third one decimal part. The maximum number it will work properly with is
    ...
    public String compressNumber(double number) {
        ...
        // The \ symbol breaks json syntax, skip it (and reduce range) (why is it not
        // being escaped by json writer?)
        if (decimalChar >= '\\')
            decimalChar++;
            ...
            TabPlayer player)
    {
        ...
        try {
            ...
            TAB.getInstance().getConfigHelper().runtime().invalidInputForNumericSorting(this, placeholder, output,
                    player);
            return defaultValue;
