...
/**
 * Cancelling GameMode change packet to spectator GameMode to avoid players
 * being moved on the bottom of TabList with transparent name. Does not work on
 * self as that would result in players not being able to clip through walls.
 */
@Getter
        ...
        ServerSwitchListener, WorldSwitchListener, VanishListener
{
     ...
     *
     * @param viewer       Player to send gamemode updates to
     * @param realGameMode Whether real GameMode should be shown or fake one
     * @param mutually     If target's view should be updated as well
     */
    private void updatePlayer(@NotNull TabPlayer viewer, boolean realGameMode, boolean mutually) {
        ...
        for (TabPlayer target : TAB.getInstance().getOnlinePlayers()) {
            ...
            if (mutually && viewer.getGamemode() == 3
    ...
    public void onWorldChange(@NotNull TabPlayer changed, @NotNull String from, @NotNull String to) {
        ...
        // Some server versions may resend gamemode on world switch, resend false value
        // again
        if (changed.getGamemode() != 3)
            return;
        for (TabPlayer viewer : TAB.getInstance().getOnlinePlayers()) {

            if (viewer == changed || viewer.hasPermission(TabConstants.Permission.SPECTATOR_BYPASS))
                continue;
    ...
    public void onVanishStatusChange(@NotNull TabPlayer player) {
        ...
        for (TabPlayer viewer : TAB.getInstance().getOnlinePlayers()) {
            ...
            if (viewer == player || viewer.hasPermission(TabConstants.Permission.SPECTATOR_BYPASS))
                continue;
            viewer.getTabList().updateGameMode(player.getTablistId(), 0);
