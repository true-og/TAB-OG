...
public class UniversalPlaceholderRegistry {
    ...
    private void registerConstants(@NotNull PlaceholderManager manager) {
        ...
        manager.registerPlayerPlaceholder(TabConstants.Placeholder.PLAYER_VERSION, -1, p -> ((TabPlayer) p)
                .getVersion()
                .getFriendlyName());
        manager.registerPlayerPlaceholder(TabConstants.Placeholder.PLAYER_VERSION_ID, -1, p -> ((TabPlayer) p)
                .getVersion()
                .getNetworkId());
        ...
        manager.registerServerPlaceholder(
                TabConstants.Placeholder.MEMORY_MAX_GB,
                -1,
    ...
    private void registerServerPlaceholders(@NotNull PlaceholderManager manager) {
        ...
        SimpleDateFormat timeFormat = createDateFormat(
                TAB.getInstance()
                        .getConfiguration()
                        .getConfig()
                        .getString("placeholders.time-format", "[HH:mm:ss / h:mm a]"),
                "[HH:mm:ss / h:mm a]");
        SimpleDateFormat dateFormat = createDateFormat(
                ...
                "dd.MM.yyyy");
        manager.registerServerPlaceholder(
                TabConstants.Placeholder.TIME,
                500,
                () -> timeFormat.format(new Date(System.currentTimeMillis() + (int) (timeOffset * 3600000))));
        manager.registerServerPlaceholder(
                TabConstants.Placeholder.DATE,
                60000,
                () -> dateFormat.format(new Date(System.currentTimeMillis() + (int) (timeOffset * 3600000))));
        manager.registerServerPlaceholder(TabConstants.Placeholder.MEMORY_USED, 200, () -> ((int)
                ((Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory()) / 1024 / 1024)));
        manager.registerServerPlaceholder(
                TabConstants.Placeholder.MEMORY_USED_GB,
                200,
                () -> decimal2.format((float) (Runtime.getRuntime().totalMemory()
    ...
    private void registerPlayerPlaceholders(@NotNull PlaceholderManager manager) {
        ...
        if (LuckPermsHook.getInstance().isInstalled()) {
            ...
            manager.registerPlayerPlaceholder(
                    TabConstants.Placeholder.LUCKPERMS_PREFIX, refresh, p -> LuckPermsHook.getInstance()
                            .getPrefix((TabPlayer) p));
            manager.registerPlayerPlaceholder(
                    TabConstants.Placeholder.LUCKPERMS_SUFFIX, refresh, p -> LuckPermsHook.getInstance()
                            .getSuffix((TabPlayer) p));
        }
        for (Object s : TAB.getInstance()
                .getConfiguration()
                .getAnimationFile()
                .getValues()
                .keySet()) {
            ...
            manager.registerPlayerPlaceholder(
                    TabConstants.Placeholder.animation(a.getName()), a.getRefresh(), p -> a.getMessage());
        }
        Condition.clearConditions();
        Map<String, Map<Object, Object>> conditions =
                TAB.getInstance().getConfiguration().getConfig().getConfigurationSection("conditions");
        for (Entry<String, Map<Object, Object>> condition : conditions.entrySet()) {
            ...
            manager.registerPlayerPlaceholder(
                    TabConstants.Placeholder.condition(c.getName()), c.getRefresh(), p -> c.getText((TabPlayer) p));
        }
