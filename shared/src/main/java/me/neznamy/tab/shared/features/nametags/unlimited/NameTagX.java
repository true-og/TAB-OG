...
public abstract class NameTagX extends NameTag implements UnlimitedNameTagManager {
    ...
    // config options
    private final boolean disableOnBoats = config()
            .getBoolean("scoreboard-teams.unlimited-nametag-mode.disable-on-boats", true);
    private final List<String> dynamicLines = new ArrayList<>(config().getStringList(
            "scoreboard-teams.unlimited-nametag-mode.dynamic-lines", Arrays.asList(TabConstants.Property.ABOVENAME,
                    TabConstants.Property.NAMETAG, TabConstants.Property.BELOWNAME, "another")));
    private final Map<String, Object> staticLines = config()
            .getConfigurationSection("scoreboard-teams.unlimited-nametag-mode.static-lines");
    private final boolean armorStandsAlwaysVisible = TAB.getInstance().getConfiguration()
            .getSecretOption("scoreboard-teams.unlimited-nametag-mode.always-visible", false);
    ...
    protected NameTagX(@NonNull BiFunction<NameTagX, TabPlayer, ArmorStandManager> armorStandFunction) {
        ...
        Condition disableCondition = Condition
                .getCondition(config().getString("scoreboard-teams.unlimited-nametag-mode.disable-condition"));
        unlimitedDisableChecker = new DisableChecker(getExtraFeatureName(), disableCondition,
                this::onUnlimitedDisableConditionChange, p -> p.disabledUnlimitedNametags);
        TAB.getInstance().getFeatureManager().registerFeature(TabConstants.Feature.UNLIMITED_NAME_TAGS + "-Condition",
                unlimitedDisableChecker);

    }

    public boolean isPlayerDisabled(@NonNull TabPlayer p) {

        return p.disabledNametags.get() || p.disabledUnlimitedNametags.get() || hasTeamHandlingPaused(p)
                || hasDisabledArmorStands(p);
    ...
    public void toggleNameTagPreview(TabPlayer player, boolean sendToggleMessage) {
        ...
        TAB.getInstance().getPlaceholderManager().getTabExpansion().setNameTagPreview(player,
                player.unlimitedNametagData.previewing);
    ...
    public boolean updateProperties(@NonNull TabPlayer p) {
        ...
        if (p.loadPropertyFromConfig(this, TabConstants.Property.CUSTOMTAGNAME, p.getName()))
            changed = true;
        if (p.setProperty(this, TabConstants.Property.NAMETAG,
                p.getProperty(TabConstants.Property.TAGPREFIX).getCurrentRawValue()
                        + p.getProperty(TabConstants.Property.CUSTOMTAGNAME).getCurrentRawValue()
                        + p.getProperty(TabConstants.Property.TAGSUFFIX).getCurrentRawValue()))
            changed = true;
    ...
    public boolean getTeamVisibility(@NonNull TabPlayer p, @NonNull TabPlayer viewer) {
        ...
        if (p.hasInvisibilityPotion())
            return false; // 1.8.x client sided bug
        if (viewer.teamData.invisibleNameTagView)
            return false;
    ...
    public void hideNameTag(@NonNull me.neznamy.tab.api.TabPlayer player,
            @NonNull me.neznamy.tab.api.TabPlayer viewer)
    {
    ...
    private void rebuildNameTagLine(@NonNull TabPlayer player) {
        ...
        player.setProperty(this, TabConstants.Property.NAMETAG,
                player.getProperty(TabConstants.Property.TAGPREFIX).getCurrentRawValue()
                        + player.getProperty(TabConstants.Property.CUSTOMTAGNAME).getCurrentRawValue()
                        + player.getProperty(TabConstants.Property.TAGSUFFIX).getCurrentRawValue());
    ...
    public void disableArmorStands(@NonNull me.neznamy.tab.api.TabPlayer player) {
        ...
        TabPlayer p = (TabPlayer) player;
        p.ensureLoaded();
        if (p.unlimitedNametagData.disabledWithAPI)
            return;
    ...
    public void setLine(@NonNull me.neznamy.tab.api.TabPlayer player, @NonNull String line, @Nullable String value) {
        ...
        TabPlayer p = (TabPlayer) player;
        p.ensureLoaded();
        if (!getDefinedLines().contains(line))
            throw new IllegalArgumentException(
                    "\"" + line + "\" is not a defined line. Defined lines: " + getDefinedLines());
