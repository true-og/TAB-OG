...
/**
 * Feature registration which offers calls to all features and measures how long
...
public class FeatureManager {
    ...
    /**
     * Flag tracking presence of a feature listening to raw packets for faster check
     * with better performance
     */
    private boolean hasPacketSendListener;

    /**
     * Flag tracking presence of a feature listening to latency change for faster
     * check with better performance
     */
    private boolean hasLatencyChangeListener;

    /**
     * Flag tracking presence of a feature listening to command preprocess for
     * faster check with better performance
     */
    ...
    public void load() {
        ...
        for (TabFeature f : values) {
            ...
            ((Loadable) f).load();
            TAB.getInstance().debug("Feature " + f.getClass().getSimpleName() + " processed load in "
    ...
    public void unload() {
        ...
        for (TabFeature f : values) {
            ...
            long time = System.currentTimeMillis();
            ((UnLoadable) f).unload();
            TAB.getInstance().debug("Feature " + f.getClass().getSimpleName() + " processed unload in "
                    + (System.currentTimeMillis() - time) + "ms");
    ...
    public void onGameModeChange(@NotNull TabPlayer player) {
        ...
        for (TabFeature f : values) {
            ...
            TAB.getInstance().getCPUManager().addTime(f.getFeatureName(), TabConstants.CpuUsageCategory.GAMEMODE_CHANGE,
    ...
    public void onQuit(@Nullable TabPlayer disconnectedPlayer) {
        ...
        for (TabFeature f : values) {
            ...
            TAB.getInstance().getCPUManager().addTime(f.getFeatureName(), TabConstants.CpuUsageCategory.PLAYER_QUIT,
                    System.nanoTime() - time);

        }

        TAB.getInstance().removePlayer(disconnectedPlayer);
        TAB.getInstance().debug("Player quit of " + disconnectedPlayer.getName() + " processed in "
    ...
    public void onJoin(@NotNull TabPlayer connectedPlayer) {
        ...
        for (TabFeature f : values) {
            ...
            TAB.getInstance().getCPUManager().addTime(f.getFeatureName(), TabConstants.CpuUsageCategory.PLAYER_JOIN,
                    System.nanoTime() - time);
            TAB.getInstance().debug("Feature " + f.getClass().getSimpleName() + " processed player join in "
                    + (System.nanoTime() - time) / 1000000 + "ms");

        }

        connectedPlayer.markAsLoaded(true);
        TAB.getInstance().debug("Player join of " + connectedPlayer.getName() + " processed in "
                + (System.currentTimeMillis() - millis) + "ms");
        if (TAB.getInstance().getConfiguration().getUsers() instanceof MySQLUserConfiguration) {
    ...
    public void onWorldChange(@NotNull UUID playerUUID, @NotNull String to) {
        ...
        for (TabFeature f : values) {
            ...
            TAB.getInstance().getCPUManager().addTime(f.getFeatureName(), TabConstants.CpuUsageCategory.WORLD_SWITCH,
                    System.nanoTime() - time);

        }

        ((PlayerPlaceholder) TAB.getInstance().getPlaceholderManager().getPlaceholder(TabConstants.Placeholder.WORLD))
                .updateValue(changed, to);
    ...
    public void onServerChange(@NotNull UUID playerUUID, @NotNull String to) {
        ...
        for (TabFeature f : values) {
            ...
            TAB.getInstance().getCPUManager().addTime(f.getFeatureName(), TabConstants.CpuUsageCategory.SERVER_SWITCH,
                    System.nanoTime() - time);

        }

        ((PlayerPlaceholder) TAB.getInstance().getPlaceholderManager().getPlaceholder(TabConstants.Placeholder.SERVER))
                .updateValue(changed, to);
    ...
    public boolean onCommand(@Nullable TabPlayer sender, @NotNull String command) {
        ...
        if (!hasCommandListener || sender == null)
            return false;
        if (!listeningCommands.contains(command))
            return false;
        boolean cancel = false;
        for (TabFeature f : values) {

            if (!(f instanceof CommandListener))
                continue;
            long time = System.nanoTime();
            if (((CommandListener) f).onCommand(sender, command))
                cancel = true;
            TAB.getInstance().getCPUManager().addTime(f.getFeatureName(),
                    TabConstants.CpuUsageCategory.COMMAND_PREPROCESS, System.nanoTime() - time);
    ...
    public void onPacketSend(@NotNull TabPlayer receiver, @NotNull Object packet) {
        ...
        if (!hasPacketSendListener)
            return;
        for (TabFeature f : values) {

            if (!(f instanceof PacketSendListener))
                continue;
            long time = System.nanoTime();
            ((PacketSendListener) f).onPacketSend(receiver, packet);
            TAB.getInstance().getCPUManager().addTime(f.getFeatureName(), TabConstants.CpuUsageCategory.RAW_PACKET_OUT,
                    System.nanoTime() - time);
    ...
    public void onDisplayObjective(@NotNull TabPlayer packetReceiver, int slot, @NotNull String objective) {
        ...
        for (TabFeature f : values) {
            ...
            TAB.getInstance().getCPUManager().addTime(f.getFeatureName(), TabConstants.CpuUsageCategory.ANTI_OVERRIDE,
                    System.nanoTime() - time);
    ...
    /**
     * Calls onObjective(TabPlayer, PacketPlayOutScoreboardObjective) on all
     * features
     *
     * @param packetReceiver player who received the packet
    ...
    public void onVanishStatusChange(@NotNull TabPlayer player) {
        ...
        for (TabFeature f : values) {
            ...
            TAB.getInstance().getCPUManager().addTime(f.getFeatureName(), TabConstants.CpuUsageCategory.VANISH_CHANGE,
                    System.nanoTime() - time);
    ...
    public void onEntryAdd(TabPlayer packetReceiver, UUID id, String name) {
        ...
        for (TabFeature f : values) {
            ...
            TAB.getInstance().getCPUManager().addTime(f.getFeatureName(),
                    TabConstants.CpuUsageCategory.NICK_PLUGIN_COMPATIBILITY, System.nanoTime() - time);
    ...
    public int onLatencyChange(TabPlayer packetReceiver, UUID id, int latency) {
        ...
        if (!hasLatencyChangeListener)
            return latency;
        int newLatency = latency;
        for (TabFeature f : values) {

            if (!(f instanceof LatencyListener))
                continue;
            long time = System.nanoTime();
            newLatency = ((LatencyListener) f).onLatencyChange(packetReceiver, id, newLatency);
            TAB.getInstance().getCPUManager().addTime(f.getFeatureName(), TabConstants.CpuUsageCategory.PING_CHANGE,
                    System.nanoTime() - time);
    ...
    public void onLoginPacket(TabPlayer packetReceiver) {
        ...
        for (TabFeature f : values) {
            ...
            if (!(f instanceof LoginPacketListener))
                continue;
            long time = System.nanoTime();
            ((LoginPacketListener) f).onLoginPacket(packetReceiver);
            TAB.getInstance().getCPUManager().addTime(f.getFeatureName(), TabConstants.CpuUsageCategory.PACKET_LOGIN,
                    System.nanoTime() - time);
    ...
    public void onTabListClear(TabPlayer packetReceiver) {
        ...
        for (TabFeature f : values) {
            ...
            TAB.getInstance().getCPUManager().addTime(f.getFeatureName(), TabConstants.CpuUsageCategory.TABLIST_CLEAR,
                    System.nanoTime() - time);
    ...
    public void loadFeaturesFromConfig() {
        ...
        boolean unlimitedTags = configuration.getConfig().getBoolean("scoreboard-teams.unlimited-nametag-mode.enabled",
                false);
        ...
        if (configuration.isPipelineInjection()) {
            ...
            if (inj != null)
                featureManager.registerFeature(TabConstants.Feature.PIPELINE_INJECTION, inj);
        ...
        if (perWorldPlayerList) {
            ...
            if (pwp != null)
                featureManager.registerFeature(TabConstants.Feature.PER_WORLD_PLAYER_LIST, pwp);
        ...
        // Must be loaded after: Global PlayerList, PlayerList, NameTags, YellowNumber,
        // BelowName
