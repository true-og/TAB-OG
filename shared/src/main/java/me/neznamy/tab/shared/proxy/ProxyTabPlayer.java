...
/**
 * Abstract class for player on proxy containing variables and methods shared
 * between proxies.
 */
@Getter
@Setter
public abstract class ProxyTabPlayer extends TabPlayer {
    ...
    /**
     * Timestamp when join plugin message was sent to track how long bridge took to
     * respond
     */
    ...
    /**
     * Constructs new instance with given parameters and sends a message to bridge
     * about this player joining with join data
     *
     ...
     * @param protocolVersion Player's protocol network id
     */
            ...
            @NotNull String name, @NotNull String server, int protocolVersion)
    {
        ...
        super(platform, player, uniqueId, name, server, "N/A", protocolVersion,
    ...
    /**
     * Sends plugin message to backend server that this player has joined,
     * containing all plugin configuration data.
     */
    public void sendJoinPluginMessage() {
        ...
        if (nametagx != null) {
            ...
            settings = new PlayerJoin.UnlimitedNametagSettings(nametagx.isDisableOnBoats(),
                    nametagx.isArmorStandsAlwaysVisible(), disabledNametags.get() || disabledUnlimitedNametags.get(),
                    nametagx.getDynamicLines(), nametagx.getStaticLines());

        }

        sendPluginMessage(new PlayerJoin(getVersion().getNetworkId(),
                TAB.getInstance().getGroupManager().getPermissionPlugin().contains("Vault")
                        && !TAB.getInstance().getGroupManager().isGroupsByPermissions(),
                ((ProxyPlatform) getPlatform()).getBridgePlaceholders(), TAB.getInstance().getConfiguration()
                        .getConfig().getConfigurationSection("placeholder-output-replacements"),
                settings));
