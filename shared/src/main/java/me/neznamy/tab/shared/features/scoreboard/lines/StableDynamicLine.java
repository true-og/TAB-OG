...
/**
 * Line of text with placeholder support Limitations: 1.5.x - 1.12.x: 28 - 32
 * characters (depending on used magic codes) 1.13+: unlimited
 */
public class StableDynamicLine extends ScoreboardLine {
    ...
    public void refresh(@NotNull TabPlayer refreshed, boolean force) {
        ...
        if (!parent.getPlayers().contains(refreshed))
            return; // player has different scoreboard displayed
    ...
    /**
     * Applies all placeholders and splits the result into prefix/suffix based on
     * client version or hides the line entirely if result is empty (and shows back
     * once it's not)
     *
     * @param p              player to replace text for
     * @param force          if action should be done despite update seemingly not
     *                       needed
    ...
    private String[] replaceText(TabPlayer p, boolean force, boolean suppressToggle) {
        ...
        if (scoreProperty == null)
            return EMPTY_ARRAY; // not actually loaded yet (force refresh called from placeholder manager
                                // register
        // method)
        ...
        if (!p.getVersion().supportsRGB()) {
            ...
            replaced = RGBUtils.getInstance().convertRGBtoLegacy(replaced); // converting RGB to legacy here to avoid
                                                                            // splitting in the middle of RGB code
    ...
    /**
     * Splits text into 2 values (prefix/suffix) based on client version and text
     * itself
     *
