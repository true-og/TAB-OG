...
public class YamlConfigurationFile extends ConfigurationFile {
     ...
     * @throws YAMLException            if file has invalid YAML syntax
    ...
    public YamlConfigurationFile(@Nullable InputStream source, @NonNull File destination) throws IOException {
        ...
        try {
            ...
            tab.getPlatform().logInfo(TabComponent
                    .fromColoredText(EnumChatFormat.GOLD + "Error message from yaml parser: " + e.getMessage()));
            List<String> suggestions = YamlAssist.getSuggestions(file);
            if (!suggestions.isEmpty()) {

                tab.getPlatform().logInfo(
                        TabComponent.fromColoredText(EnumChatFormat.LIGHT_PURPLE + "Suggestions to fix yaml syntax:"));
                for (String suggestion : suggestions) {

                    tab.getPlatform()
                            .logInfo(TabComponent.fromColoredText(EnumChatFormat.LIGHT_PURPLE + "- " + suggestion));
    ...
    public void save() {
        ...
        try {
            ...
            TAB.getInstance().getPlatform().logWarn(TabComponent.fromColoredText(
                    "Failed to save yaml file " + file.getPath() + " with content " + values.toString()));
