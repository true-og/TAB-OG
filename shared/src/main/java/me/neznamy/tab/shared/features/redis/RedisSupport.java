...
/**
 * Feature synchronizing player display data between multiple proxies connected
 * with a redis plugin.
 */
@SuppressWarnings("UnstableApiUsage")
@Getter
public abstract class RedisSupport extends TabFeature implements JoinListener, QuitListener, Loadable, UnLoadable,
        ServerSwitchListener, LoginPacketListener, VanishListener, TabListClearListener
{
     ...
     *
     * @param p      Player to format
     * @param format Format to use
     */
    public void updateTabFormat(@NotNull TabPlayer p, @NotNull String format) {

        if (redisPlayerList == null)
            return; // Plugin still loading
     ...
     * @param tagPrefix      Team prefix
     * @param tagSuffix      Team suffix
     * @param nameVisibility Nametag visibility
     */
     ...
     *
     * @param p          Player to update
     * @param value      Numeric value of player
     * @param fancyValue NumberFormat value of player
     */
        ...
        TAB.getInstance().getCPUManager().runMeasuredTask(getFeatureName(),
                TabConstants.CpuUsageCategory.REDIS_BUNGEE_MESSAGE, () ->
                {
                    ...
                    if (proxy.equals(this.proxy.toString()))
                        return; // Message coming from current proxy
    ...
    public void load() {
        ...
        if (TAB.getInstance().getFeatureManager().isFeatureEnabled(TabConstants.Feature.PLAYER_LIST)) {
            ...
            redisPlayerList = new RedisPlayerList(this,
        ...
        if (TAB.getInstance().getFeatureManager().isFeatureEnabled(TabConstants.Feature.GLOBAL_PLAYER_LIST)) {
            ...
            features.add(new RedisGlobalPlayerList(this,
                    TAB.getInstance().getFeatureManager().getFeature(TabConstants.Feature.GLOBAL_PLAYER_LIST)));
    ...
    private void overridePlaceholders() {
        ...
        TAB.getInstance().getPlaceholderManager().registerServerPlaceholder(TabConstants.Placeholder.STAFF_ONLINE, 1000,
                () ->
                {
                    ...
                    for (TabPlayer player : TAB.getInstance().getOnlinePlayers()) {
                        ...
                        if (!player.isVanished() && player.hasPermission(TabConstants.Permission.STAFF))
                            count++;
        ...
        TAB.getInstance().getPlaceholderManager().registerPlayerPlaceholder(TabConstants.Placeholder.SERVER_ONLINE,
                1000, p ->
                {
     ...
     */
    public void registerMessage(@NotNull String name, @NotNull Class<? extends RedisMessage> clazz,
            @NotNull Supplier<RedisMessage> supplier)
    {
