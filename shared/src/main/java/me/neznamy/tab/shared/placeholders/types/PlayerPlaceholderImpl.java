...
public class PlayerPlaceholderImpl extends TabPlaceholder implements PlayerPlaceholder {
    ...
    /**
     * Last known values for each online player after applying replacements and
     * nested placeholders
     */
     ...
     *                   request
     */
    public PlayerPlaceholderImpl(@NonNull String identifier, int refresh,
     ...
     * {@code false} if not.
     *
     * @param p     Player to update value for
    ...
    public Object request(@NonNull TabPlayer p) {
        ...
        try {
            ...
            TAB.getInstance().getErrorManager().placeholderError(
                    "Player placeholder " + identifier + " generated an error when setting for player " + p.getName(),
                    t);
            return ERROR_VALUE;
            ...
            if (timeDiff > TabConstants.Placeholder.RETURN_TIME_WARN_THRESHOLD) {
                ...
                TAB.getInstance().debug("Placeholder " + identifier + " took " + timeDiff
                        + "ms to return value for player " + p.getName());
