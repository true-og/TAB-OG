...
/**
 * Abstract class storing common variables and functions for player, which are
...
public abstract class TabPlayer implements me.neznamy.tab.api.TabPlayer {
    ...
    /**
     * World the player is currently in, {@code "N/A"} if TAB is installed on proxy
     * and bukkit bridge is not installed
     */
    @Getter
    @Setter
    ...
    /**
     * Server the player is currently in, {@code "N/A"} if TAB is installed on
     * Bukkit
     */
    @Getter
    @Setter
    ...
    /**
     * Player's permission group defined in permission plugin or with permission
     * nodes
     */
    ...
    /**
     * Player's property map where key is unique identifier and value is property
     * object
     */
    ...
    /**
     * Player's load status, {@code true} when player is fully loaded, {@code false}
     * if not yet
     */
    @Getter
     ...
     * @param useRealId       Whether tablist uses real uuid or offline
     */
    protected TabPlayer(@NotNull Platform platform, @NotNull Object player, @NotNull UUID uniqueId,
            @NotNull String name, @NotNull String server, @NotNull String world, int protocolVersion, boolean useRealId)
    {
     ...
     * @param source     Source of raw value
     * @return {@code true} if property did not exist or existed with different raw
     *         value, {@code false} if property existed with the same raw value
     *         already.
     */
            ...
            @Nullable String source, boolean exposeInExpansion)
    {
        ...
        if (p == null) {
            ...
            properties.put(identifier,
                    new Property(exposeInExpansion ? identifier : null, feature, this, rawValue, source));
            return true;
    ...
    /**
     * Sets property with specified name to new value. If property did not exist
     * before, it is created and {@code true} is returned. If it existed, it is
     * overridden and {@code true} is returned. {@code false} is returned otherwise.
     *
     * @param feature    feature using this property to get placeholders registered
     * @param identifier property name
     * @param rawValue   new raw value
     * @return {@code true} if value changed / did not exist, {@code false} if value
     *         did not change
     */
     ...
     *
     * @param join {@code true} if this is a player join, {@code false} if reload
     */
     ...
     *
     * @param permissionGroup New permission group
     */
    ...
    public void setTemporaryGroup(@Nullable String group) {
        ...
        if (Objects.equals(group, temporaryGroup))
            return;
    ...
    public void setExpectedProfileName(@NonNull String profileName) {
        ...
        NickCompatibility nick = TAB.getInstance().getFeatureManager()
                .getFeature(TabConstants.Feature.NICK_COMPATIBILITY);
     ...
     *
     * @param feature  Feature using this property
     * @param property property name to load
     * @return {@code true} if value did not exist or changed, {@code false}
     *         otherwise
     */
    ...
    /**
     * Loads property from config using standard property loading algorithm. If the
     * property is not set in config, {@code ifNotSet} value is used.
     *
     * @param feature  Feature using this property
     * @param property property name to load
     ...
     */
    public boolean loadPropertyFromConfig(@Nullable Refreshable feature, @NotNull String property,
            @NotNull String ifNotSet)
    {

        String[] value = TAB.getInstance().getConfiguration().getUsers().getProperty(name, property, server, world);
        if (value.length == 0) {

            value = TAB.getInstance().getConfiguration().getUsers().getProperty(uniqueId.toString(), property, server,
                    world);
    ...
    /**
     * Makes sure the player is loaded. If not, throws
     * {@link IllegalStateException}.
     *
     * @throws IllegalStateException If player is not loaded yet
     */
    ...
    /**
     * Checks if player is vanished using any supported vanish plugin and returns
     * the result.
     *
    ...
    /**
     * Returns GameMode of the player (0 for survival, 1 creative, 2 adventure, 3
     * spectator)
     *
    ...
    /**
     * Performs platform-specific API call to check for permission and returns the
     * result
     *
