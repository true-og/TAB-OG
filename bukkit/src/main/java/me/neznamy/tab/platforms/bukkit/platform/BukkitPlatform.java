...
public class BukkitPlatform implements BackendPlatform {
    ...
    /** Server version */
    @Getter
    private final ProtocolVersion serverVersion = ProtocolVersion
            .fromFriendlyName(Bukkit.getBukkitVersion().split("-")[0]);
    ...
    /**
     * Spigot field for tracking TPS, the array is final and only being modified
    ...
    public BukkitPlatform(@NotNull JavaPlugin plugin) {
        ...
        Bukkit.getConsoleSender().sendMessage(
                "[TAB] " + EnumChatFormat.GRAY + "Loaded NMS hook in " + (System.currentTimeMillis() - time) + "ms");
    ...
    public void registerPlaceholders() {
        ...
        if (Bukkit.getPluginManager().isPluginEnabled("Vault")) {
            ...
            if (rspChat != null) {
                ...
                manager.registerPlayerPlaceholder("%vault-prefix%", refresh,
                        p -> chat.getPlayerPrefix((Player) p.getPlayer()));
                manager.registerPlayerPlaceholder("%vault-suffix%", refresh,
                        p -> chat.getPlayerSuffix((Player) p.getPlayer()));

            }

        }

        // Override for the PAPI placeholder to prevent console errors on unsupported
        // server versions when ping field
        // changes
    ...
    public PipelineInjector createPipelineInjector() {
        ...
        return BukkitReflection.getMinorVersion() >= 8 && BukkitPipelineInjector.isAvailable()
    ...
    public NameTag getUnlimitedNameTags() {
        ...
        return BukkitReflection.getMinorVersion() >= 8 && PacketEntityView.isAvailable()
                && BukkitPipelineInjector.isAvailable() ? new BukkitNameTagX(plugin) : new NameTag();
    ...
    public void registerUnknownPlaceholder(@NotNull String identifier) {
        ...
        if (identifier.startsWith("%rel_")) {
            ...
            TAB.getInstance().getPlaceholderManager().registerRelationalPlaceholder(identifier,
                    pl.getRefreshInterval(identifier),
                    (viewer, target) -> PlaceholderAPI.setRelationalPlaceholders((Player) viewer.getPlayer(),
                            (Player) target.getPlayer(), identifier));
     ...
     * @param refresh    Placeholder refresh
     */
    public void registerSyncPlaceholder(@NotNull String identifier, int refresh) {
        ...
        ppl[0] = TAB.getInstance().getPlaceholderManager().registerPlayerPlaceholder(identifier, refresh, p -> {
            ...
            Bukkit.getScheduler().runTask(plugin, () -> {
                ...
                ppl[0].updateValue(p,
                        placeholderAPI ? PlaceholderAPI.setPlaceholders((Player) p.getPlayer(), syncedPlaceholder)
                                : identifier);
    ...
    public GroupManager detectPermissionPlugin() {
        ...
        if (Bukkit.getPluginManager().isPluginEnabled("Vault")) {
            ...
            RegisteredServiceProvider<Permission> provider = Bukkit.getServicesManager()
    ...
    public boolean canSee(@NotNull TabPlayer viewer, @NotNull TabPlayer target) {
        ...
        if (BackendPlatform.super.canSee(viewer, target))
            return true;
    ...
    /**
     * Converts component to legacy string using bukkit RGB format if supported by
     * both server and client. If not, closest legacy color is used instead.
     *
     * @param component Component to convert
     * @param rgbClient Whether client accepts RGB colors or not.
        ...
        if (iComponent.getModifier().getColor() != null) {
            ...
            if (serverVersion.supportsRGB() && rgbClient) {
                ...
                sb.append(c).append("x").append(c).append(hexCode.charAt(0)).append(c).append(hexCode.charAt(1))
                        .append(c).append(hexCode.charAt(2)).append(c).append(hexCode.charAt(3)).append(c)
                        .append(hexCode.charAt(4)).append(c).append(hexCode.charAt(5));
