...
public class BossBarLine implements BossBar {
     ...
     * @param announcementOnly Whether this bossbar is for announcements only
     */
    public BossBarLine(@NonNull BossBarManagerImpl manager, @NonNull String name, @Nullable String displayCondition,
            @NonNull String color, @NonNull String style, @NonNull String title, @NonNull String progress,
    ...
    /**
     * Parses string into color and returns it. If parsing failed, PURPLE is
     * returned.
     *
    ...
    /**
     * Parses string into style and returns it. If parsing failed, PROGRESS is
     * returned.
     *
    ...
    /**
     * Parses string into progress and returns it. If parsing failed, 100 is
     * returned instead and error message is printed into error log
     *
    ...
    public float parseProgress(@NotNull TabPlayer player, @NotNull String progress) {
        ...
        try {
            ...
            TAB.getInstance().getConfigHelper().runtime().invalidNumberForBossBarProgress(this, progress,
                    player.getProperty(propertyProgress).getCurrentRawValue(), player);
            return 100;
     ...
     *
     * @param player Player to resend bossbar to
     */
    public void sendToPlayerRaw(@NotNull TabPlayer player) {

        player.getBossBar().create(uniqueId, player.getProperty(propertyTitle).updateAndGet(),
    ...
    private class ProgressRefresher extends TabFeature implements Refreshable {
        ...
        public void refresh(@NotNull TabPlayer refreshed, boolean force) {
            ...
            if (!players.contains(refreshed))
                return;
            refreshed.getBossBar().update(uniqueId,
                    parseProgress(refreshed, refreshed.getProperty(propertyProgress).updateAndGet()) / 100);
